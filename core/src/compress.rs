...
pub fn register_compress_globals(
                ...
                move |lua, (first, archive_opt): (mlua::Value, Option<String>)| {
                    ...
                    let (source_val, archive) = if let Some(a) = archive_opt {
                        ...
                        let arc = t
                            ...
                            .map_err(|_| {
                                mlua::Error::external("compress.zip: missing archive argument")
                            })?;
                        let src_val = if let Ok(s) = t.get::<String>(1) {
                            mlua::Value::String(lua.create_string(&s)?)
                        } else {
                            t.get::<mlua::Value>("source").map_err(|_| {
                                mlua::Error::external("compress.zip: missing source argument")
                            })?
                        };
                        (src_val, arc)
                    } else {
                        return Err(mlua::Error::external(
                            "compress.zip: missing archive argument",
                        ));
                    };
                    let archive_host = m.resolve_write(&archive).map_err(mlua::Error::external)?;
    ...
    // compress.unzip(archive, dest)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                let validated =
                    validate_args(&args, COMPRESS_DOC.params("unzip"), "compress.unzip")?;
                let archive = match &validated[0] {
                    mlua::Value::String(s) => s.to_string_lossy().to_string(),
                    _ => unreachable!(),
                };
                let dest = match &validated[1] {
                    mlua::Value::String(s) => s.to_string_lossy().to_string(),
                    _ => unreachable!(),
                };
                let archive_host = m.resolve_read(&archive).map_err(mlua::Error::external)?;
                let dest_host = m.resolve_write_deep(&dest).map_err(mlua::Error::external)?;
                std::fs::create_dir_all(&dest_host).map_err(mlua::Error::external)?;
                extract_zip(&archive_host, &dest_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
    ...
    // compress.bzip2(input, output)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                let validated =
                    validate_args(&args, COMPRESS_DOC.params("bzip2"), "compress.bzip2")?;
                let input = match &validated[0] {
                    mlua::Value::String(s) => s.to_string_lossy().to_string(),
                    _ => unreachable!(),
                };
                let output = match &validated[1] {
                    mlua::Value::String(s) => s.to_string_lossy().to_string(),
                    _ => unreachable!(),
                };
                ...
                bzip2_compress(&input_host, &output_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
    ...
    // compress.bunzip2(input, output)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                let validated =
                    validate_args(&args, COMPRESS_DOC.params("bunzip2"), "compress.bunzip2")?;
                let input = match &validated[0] {
                    mlua::Value::String(s) => s.to_string_lossy().to_string(),
                    _ => unreachable!(),
                };
                ...
                bzip2_decompress(&input_host, &output_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
    ...
    // compress.xz(input, output)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                xz_compress(&input_host, &output_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
    ...
    // compress.unxz(input, output)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                xz_decompress(&input_host, &output_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
    ...
    // compress.un7z(archive, dest)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                let dest_host = m.resolve_write_deep(&dest).map_err(mlua::Error::external)?;
                std::fs::create_dir_all(&dest_host).map_err(mlua::Error::external)?;
                extract_7z(&archive_host, &dest_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
                ...
                move |lua, (first, archive_opt): (mlua::Value, Option<String>)| {
                    ...
                    let (source_val, archive) = if let Some(a) = archive_opt {
                        ...
                        let arc = t
                            ...
                            .or_else(|_| t.get::<String>("archive"))
                            .map_err(|_| {
                                mlua::Error::external("compress.tar: missing archive argument")
                            })?;
                        let src_val = if let Ok(s) = t.get::<String>(1) {
                            mlua::Value::String(lua.create_string(&s)?)
                        } else {
                            t.get::<mlua::Value>("source").map_err(|_| {
                                mlua::Error::external("compress.tar: missing source argument")
                            })?
                        };
                        (src_val, arc)
                    } else {
                        return Err(mlua::Error::external(
                            "compress.tar: missing archive argument",
                        ));
                    };
                    let archive_host = m.resolve_write(&archive).map_err(mlua::Error::external)?;
                    if let Some(parent) = archive_host.parent() {
                    ...
                    create_tar(&sources, &archive_host).map_err(mlua::Error::RuntimeError)?;
                    Ok(())
                },
            )?,
        )?;
    ...
    // compress.untar(archive, dest)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                let validated =
                    validate_args(&args, COMPRESS_DOC.params("untar"), "compress.untar")?;
                let archive = match &validated[0] {
                    mlua::Value::String(s) => s.to_string_lossy().to_string(),
                    _ => unreachable!(),
                };
                ...
                let dest_host = m.resolve_write_deep(&dest).map_err(mlua::Error::external)?;
                std::fs::create_dir_all(&dest_host).map_err(mlua::Error::external)?;
                extract_tar(&archive_host, &dest_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
                ...
                move |lua, (first, archive_opt): (mlua::Value, Option<String>)| {
                    ...
                    let (source_val, archive) = if let Some(a) = archive_opt {
                        ...
                        let arc = t
                            ...
                            .or_else(|_| t.get::<String>("archive"))
                            .map_err(|_| {
                                mlua::Error::external("compress.targz: missing archive argument")
                            })?;
                        let src_val = if let Ok(s) = t.get::<String>(1) {
                            mlua::Value::String(lua.create_string(&s)?)
                        } else {
                            t.get::<mlua::Value>("source").map_err(|_| {
                                mlua::Error::external("compress.targz: missing source argument")
                            })?
                        };
                        (src_val, arc)
                    } else {
                        return Err(mlua::Error::external(
                            "compress.targz: missing archive argument",
                        ));
                    };
                    let archive_host = m.resolve_write(&archive).map_err(mlua::Error::external)?;
                    if let Some(parent) = archive_host.parent() {
                    ...
                    create_tar_gz(&sources, &archive_host).map_err(mlua::Error::RuntimeError)?;
                    Ok(())
                },
            )?,
        )?;
    ...
    // compress.untargz(archive, dest)
    {
        ...
        compress.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                let validated =
                    validate_args(&args, COMPRESS_DOC.params("untargz"), "compress.untargz")?;
                let archive = match &validated[0] {
                    mlua::Value::String(s) => s.to_string_lossy().to_string(),
                    _ => unreachable!(),
                };
                ...
                let dest_host = m.resolve_write_deep(&dest).map_err(mlua::Error::external)?;
                std::fs::create_dir_all(&dest_host).map_err(mlua::Error::external)?;
                extract_tar_gz(&archive_host, &dest_host).map_err(mlua::Error::RuntimeError)?;
                Ok(())
            })?,
        )?;
    }
...
fn resolve_source_paths(
    ...
    match source {
        ...
        _ => Err(mlua::Error::RuntimeError(format!(
            "{}: source must be a string or table of strings",
            fn_name
        ))),
    }
}
    ...
    sevenz_rust::decompress_file_with_extract_fn(archive, dest, |entry, reader, out_path| {
        ...
        if !out_canonical.starts_with(&dest_canonical) {
            return Err(std::io::Error::new(
                ...
                format!("7z entry '{}' escapes destination — rejected", entry.name()),
            )
            .into());
        }
...
mod tests {
    ...
    fn test_zip_mixed_files_and_dirs() {
        ...
        sandbox
            .exec(
                "compress.zip({'/workspace/solo.txt', '/workspace/subdir'}, '/workspace/mix.zip')",
            )
            .unwrap();
    ...
    fn test_bzip2_roundtrip() {
        ...
        let restored = fs::read_to_string(dir.path().join("restored.txt")).unwrap();
