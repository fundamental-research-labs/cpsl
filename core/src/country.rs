...
fn currencies_for_country(alpha2: &str) -> Vec<Currency> {
    ...
    Currency::iter()
        .filter(|cur| cur.used_by().iter().any(|c| format!("{:?}", c) == upper))
        .collect()
}
...
pub fn register_country_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    country_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, COUNTRY_DOC.params("lookup"), "country.lookup")?;
    ...
    country_table.set(
        "byName",
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, COUNTRY_DOC.params("byName"), "country.byName")?;
    ...
    country_table.set(
        "all",
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, COUNTRY_DOC.params("all"), "country.all")?;
    ...
    country_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            ...
            let cc = resolve_country(&code).ok_or_else(|| {
                mlua::Error::external(format!("country.currency: unknown country code '{}'", code))
            })?;
    ...
    currency_table.set(
        "lookup",
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, CURRENCY_DOC.params("lookup"), "currency.lookup")?;
            let code = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    currency_table.set(
        "all",
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, CURRENCY_DOC.params("all"), "currency.all")?;
            let query = match &validated[0] {
                Value::String(s) => Some(s.to_string_lossy().to_string().to_lowercase()),
                _ => None,
            };
