...
const FUZZY_EXTRACT_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc {
        ...
        required: false,
        description:
            "Scoring function: ratio (default), partial_ratio, token_sort_ratio, token_set_ratio",
    },
];
...
pub fn register_fuzzy_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    fuzzy_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(
                &args,
                FUZZY_DOC.params("partial_ratio"),
                "fuzzy.partial_ratio",
            )?;
            let s1 = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
    ...
    fuzzy_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(
                &args,
                FUZZY_DOC.params("token_sort_ratio"),
    ...
    fuzzy_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(
                &args,
                FUZZY_DOC.params("token_set_ratio"),
    ...
    fuzzy_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, FUZZY_DOC.params("extract"), "fuzzy.extract")?;
            ...
            let cutoff = opts
                ...
                .unwrap_or(0.0);
            let scorer_name = opts.as_ref().and_then(|t| {
                t.get::<String>("scorer")
                    .ok()
    ...
    fuzzy_table.set(
        "distance",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, FUZZY_DOC.params("distance"), "fuzzy.distance")?;
            let s1 = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
