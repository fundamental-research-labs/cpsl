    ...
    let country = region
        .map(|r| {
            r.to_uppercase()
                ...
                .map_err(|e| mlua::Error::external(format!("phone: invalid region '{}': {}", r, e)))
        })
        .transpose()?;
...
pub fn register_phone_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    phone_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            ...
            result.set("country_code", parsed.code().value() as i32)?;
            result.set("national_number", parsed.national().value().to_string())?;
    ...
    phone_table.set(
        "isValid",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, PHONE_DOC.params("isValid"), "phone.isValid")?;
            let number = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    phone_table.set(
        "region",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, PHONE_DOC.params("region"), "phone.region")?;
            let number = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
