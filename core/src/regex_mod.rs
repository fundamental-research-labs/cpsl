...
pub fn register_regex_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    regex_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, REGEX_DOC.params("match"), "regex.match")?;
    ...
    regex_table.set(
        "find_all",
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, REGEX_DOC.params("find_all"), "regex.find_all")?;
            let pattern = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    regex_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, REGEX_DOC.params("replace"), "regex.replace")?;
            let pattern = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    regex_table.set(
        "split",
        lua.create_function(|lua, args: MultiValue| {
            let validated = validate_args(&args, REGEX_DOC.params("split"), "regex.split")?;
            let pattern = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    regex_table.set(
        "is_match",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, REGEX_DOC.params("is_match"), "regex.is_match")?;
            let pattern = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    regex_table.set(
        "escape",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, REGEX_DOC.params("escape"), "regex.escape")?;
