...
fn lookup_invalid_returns_nil() {
    let s = sb();
    let r = s.exec(r#"return tostring(country.lookup("ZZ"))"#).unwrap();
    assert_eq!(r, "nil");
}
...
fn lookup_empty_string_returns_nil() {
    let s = sb();
    let r = s.exec(r#"return tostring(country.lookup(""))"#).unwrap();
    assert_eq!(r, "nil");
}
...
fn all_with_filter() {
    ...
    assert!(
        count >= 1,
        "expected at least 1 match for 'JP', got: {}",
        count
    );
}
...
fn currency_invalid_country_errors() {
    let s = sb();
    let err = s.exec(r#"country.currency("ZZ")"#).unwrap_err();
    assert!(
...
fn currency_lookup_empty_returns_nil() {
    let s = sb();
    let r = s.exec(r#"return tostring(currency.lookup(""))"#).unwrap();
    assert_eq!(r, "nil");
}
...
fn currency_all_filter_by_code() {
    ...
    assert!(
        count >= 1,
        "expected at least 1 match for 'usd', got: {}",
        count
    );
}
...
fn shell_country_by_name() {
    ...
    let luau = result.unwrap().luau_source;
    let r = s.exec(&luau).unwrap();
    assert!(r.contains("JP"), "expected Japan in results, got: {}", r);
}
