...
fn sha256_known_vector() {
    let s = sb();
    let r = s.exec(r#"return crypto.sha256("hello")"#).unwrap();
    assert_eq!(
        r,
...
fn sha512_known_vector() {
    let s = sb();
    let r = s.exec(r#"return crypto.sha512("hello")"#).unwrap();
    assert_eq!(
        r,
...
fn md5_known_vector() {
    let s = sb();
    let r = s.exec(r#"return crypto.md5("hello")"#).unwrap();
...
fn sha256_empty_string() {
    let s = sb();
    let r = s.exec(r#"return crypto.sha256("")"#).unwrap();
...
fn md5_returns_32_hex_chars() {
    let s = sb();
    let r = s.exec(r#"return tostring(#crypto.md5("test"))"#).unwrap();
    assert_eq!(r, "32");
}
...
fn hex_encode_hello() {
    let s = sb();
    let r = s.exec(r#"return crypto.hex_encode("hello")"#).unwrap();
...
fn hex_decode_hello() {
    let s = sb();
    let r = s.exec(r#"return crypto.hex_decode("68656c6c6f")"#).unwrap();
    assert_eq!(r, "hello");
}
...
fn hex_decode_invalid_hex_errors() {
    let s = sb();
    let err = s.exec(r#"return crypto.hex_decode("xyz")"#).unwrap_err();
    assert!(
...
fn random_bytes_zero_returns_empty() {
    let s = sb();
    let r = s.exec(r#"return crypto.random_bytes(0)"#).unwrap();
    assert_eq!(r, "");
}
...
fn sha256_table_form_positional() {
    let s = sb();
    let r = s.exec(r#"return crypto.sha256({[1]="hello"})"#).unwrap();
    assert_eq!(
        r,
        "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824"
    );
...
fn sha256_table_form_named() {
    let s = sb();
    let r = s.exec(r#"return crypto.sha256({data="hello"})"#).unwrap();
    assert_eq!(
        r,
        "2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824"
    );
...
fn hmac_sha256_missing_second_arg_errors() {
    let s = sb();
    let err = s.exec(r#"crypto.hmac_sha256("key")"#).unwrap_err();
    assert!(
        err.message.contains("bad argument") || err.message.contains("missing"),
        "msg: {}",
        err.message
...
fn decrypt_wrong_number_of_args_errors() {
    let s = sb();
    let err = s.exec(r#"crypto.decrypt("aabb")"#).unwrap_err();
    assert!(
        err.message.contains("bad argument") || err.message.contains("missing"),
        "msg: {}",
        err.message
...
fn random_bytes_non_number_errors() {
    let s = sb();
    let err = s.exec(r#"crypto.random_bytes("abc")"#).unwrap_err();
    assert!(
...
fn hex_encode_empty_string() {
    let s = sb();
    let r = s.exec(r#"return crypto.hex_encode("")"#).unwrap();
    assert_eq!(r, "");
}
...
fn uuid_v4_length() {
    let s = sb();
    let r = s.exec(r#"return tostring(#crypto.uuid())"#).unwrap();
...
fn uuid_v7_length() {
    let s = sb();
    let r = s.exec(r#"return tostring(#crypto.uuid_v7())"#).unwrap();
    assert_eq!(r, "36");
}
