...
fn parse_empty_csv() {
    let s = sb();
    let r = s
        .exec(r#"local data = csv.parse(""); return #data"#)
        .unwrap();
    assert_eq!(r, "0");
}
...
fn shell_csv_parse_transpile_flags() {
    ...
    assert!(
        ...
        "should have delimiter flag, got: {}",
        luau
    );
    assert!(
        luau.contains("header=true"),
        "should have header flag, got: {}",
        luau
    );
}
