...
fn now_returns_iso8601() {
    ...
    assert!(
        r.len() == 20,
        "expected 20 chars (YYYY-MM-DDTHH:MM:SSZ), got {} chars: {}",
        r.len(),
        r
    );
}
...
fn parse_date_only() {
    let s = sb();
    let r = s.exec(r#"return datetime.parse("2024-06-15")"#).unwrap();
...
fn parse_us_date_format() {
    let s = sb();
    let r = s.exec(r#"return datetime.parse("06/15/2024")"#).unwrap();
    assert_eq!(r, "2024-06-15T00:00:00Z");
}
...
fn parse_european_date_format() {
    let s = sb();
    let r = s.exec(r#"return datetime.parse("15.06.2024")"#).unwrap();
    assert_eq!(r, "2024-06-15T00:00:00Z");
}
...
fn diff_unknown_unit_errors() {
    ...
    let err = s
        ...
        .unwrap_err();
    assert!(err.message.contains("unknown unit"), "msg: {}", err.message);
}
...
fn fromtimestamp_epoch() {
    let s = sb();
    let r = s.exec(r#"return datetime.fromtimestamp(0)"#).unwrap();
...
fn date_basic() {
    let s = sb();
    let r = s.exec(r#"return datetime.date(2024, 6, 15)"#).unwrap();
    assert_eq!(r, "2024-06-15T00:00:00Z");
}
...
fn date_first_day_of_year() {
    let s = sb();
    let r = s.exec(r#"return datetime.date(2024, 1, 1)"#).unwrap();
    assert_eq!(r, "2024-01-01T00:00:00Z");
}
...
fn date_leap_year() {
    let s = sb();
    let r = s.exec(r#"return datetime.date(2024, 2, 29)"#).unwrap();
...
fn date_invalid_feb_29_non_leap() {
    ...
    assert!(err.message.contains("invalid date"), "msg: {}", err.message);
}
...
fn shell_datetime_now() {
    ...
    assert!(
        r.is_ok(),
        "shell datetime now should not error: {:?}",
        r.err()
    );
}
...
fn shell_datetime_parse() {
    ...
    assert!(r.contains("2024-06-15"), "expected parsed date, got: {}", r);
}
...
fn shell_datetime_timestamp() {
    ...
    s.register_module("shrt", shrt).unwrap();
    let result =
        cpsl_core::sh_transpile::transpile_sh(r#"datetime timestamp "2024-01-01T00:00:00Z""#);
    assert!(result.is_ok(), "transpile err: {:?}", result.err());
    let luau = result.unwrap().luau_source;
    let r = s.exec(&luau).unwrap();
    assert!(
...
fn fromtimestamp_wrong_type_errors() {
    let s = sb();
    let err = s
        .exec(r#"datetime.fromtimestamp("not a number")"#)
        .unwrap_err();
    assert!(
...
fn strptime_bad_format_errors() {
    let s = sb();
    let err = s
        .exec(r#"datetime.strptime("not a date", "%Y-%m-%d")"#)
        .unwrap_err();
    assert!(
        err.message.contains("could not parse"),
        "msg: {}",
...
fn fromtimestamp_negative() {
    ...
    let s = sb();
    let r = s.exec(r#"return datetime.fromtimestamp(-86400)"#).unwrap();
