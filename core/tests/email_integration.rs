...
fn is_valid_empty_string() {
    let s = sb();
    let r = s.exec(r#"return tostring(email.isValid(""))"#).unwrap();
    assert_eq!(r, "false");
}
...
fn normalize_invalid_errors() {
    let s = sb();
    let err = s.exec(r#"email.normalize("not an email")"#).unwrap_err();
    assert!(
...
fn shell_email_is_valid() {
    ...
    assert!(
        r.is_ok(),
        "shell email isValid should not error: {:?}",
        r.err()
    );
}
...
fn shell_email_normalize() {
    ...
    let shrt = include_str!("../../runtime/shrt.luau");
    s.register_module("shrt", shrt).unwrap();
    let result = cpsl_core::sh_transpile::transpile_sh(r#"email normalize "User@EXAMPLE.COM""#);
    assert!(result.is_ok(), "transpile err: {:?}", result.err());
    let luau = result.unwrap().luau_source;
    let r = s.exec(&luau).unwrap();
    assert!(
...
fn global_help_mentions_email() {
    ...
    assert!(r.contains("email"), "global help should list email: {}", r);
}
