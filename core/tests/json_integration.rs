...
fn decode_object() {
    let s = sb();
    let r = s
        .exec(r#"local t = json.decode('{"a":1,"b":"hello"}'); return t.a .. " " .. t.b"#)
        .unwrap();
    assert_eq!(r, "1 hello");
}
...
fn decode_array() {
    let s = sb();
    let r = s
        .exec(r#"local t = json.decode('[10,20,30]'); return t[1] .. "," .. t[2] .. "," .. t[3]"#)
        .unwrap();
...
fn decode_null_becomes_nil() {
    let s = sb();
    let r = s
        .exec(r#"local t = json.decode('{"a":null}'); return t.a == nil"#)
        .unwrap();
    assert_eq!(r, "true");
}
...
fn decode_empty_object() {
    ...
    let r = s
        .exec(r#"local t = json.decode('{}'); return type(t)"#)
        .unwrap();
    assert_eq!(r, "table");
}
...
fn encode_table_as_object() {
    let s = sb();
    let r = s
        .exec(r#"return json.encode({name = "test", val = 42})"#)
        .unwrap();
...
fn encode_nested() {
    let s = sb();
    let r = s
        .exec(r#"return json.encode({a = {1, 2}, b = "hi"})"#)
        .unwrap();
    let v: serde_json::Value = serde_json::from_str(&r).unwrap();
...
fn encode_pretty() {
    let s = sb();
    let r = s
        .exec(r#"return json.encode({a = 1}, {pretty = true})"#)
        .unwrap();
    assert!(
        r.contains('\n'),
...
fn json_nonexistent_fn_hint() {
    ...
    assert!(
        err.message.contains("json.foo does not exist"),
        "msg: {}",
        err.message
    );
    assert!(
        err.message.contains("hint: call json.help() for usage"),
        "msg: {}",
        err.message
    );
}
