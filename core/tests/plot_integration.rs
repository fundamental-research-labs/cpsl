...
fn bar_basic() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.bar({"A", "B", "C"}, {10, 20, 15}, {output = "/workspace/bar.html"})"#)
        .unwrap();
...
fn histogram_too_few_points_errors() {
    ...
    let err = s
        ...
        .unwrap_err();
    assert!(err.message.contains("at least 2"), "error: {}", err.message);
}
...
fn pie_basic() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.pie({"A", "B", "C"}, {30, 50, 20}, {output = "/workspace/pie.html"})"#)
        .unwrap();
...
fn pie_zero_total_errors() {
    ...
    let err = s
        ...
        .unwrap_err();
    assert!(err.message.contains("sum to > 0"), "error: {}", err.message);
}
...
fn line_table_form() {
    ...
    let r = s
        .exec(
            r#"return plot.line({x = {1, 2, 3}, y = {10, 20, 15}, output = "/workspace/lt.html"})"#,
        )
        .unwrap();
    assert_eq!(r, "/workspace/lt.html");
...
fn scatter_table_form() {
    ...
    let r = s
        .exec(
            r#"return plot.scatter({x = {1, 2, 3}, y = {4, 5, 6}, output = "/workspace/st.html"})"#,
        )
        .unwrap();
...
fn plot_line_nested_tables_shows_inline_help() {
    let (s, _dir) = sb_with_workspace();
    let err = s
        .exec(r#"plot.line({1,2,3}, {{10,20,30},{40,50,60}}, {output = "/workspace/t.html"})"#)
        .unwrap_err();
    assert!(
        err.message.contains("expected"),
        "should mention type expectation: {}",
...
fn global_help_mentions_plot() {
    ...
    assert!(r.contains("plot"), "global help should list plot: {}", r);
}
...
fn missing_output_errors() {
    let (s, _dir) = sb_with_workspace();
    let err = s.exec(r#"plot.line({1, 2, 3}, {4, 5, 6})"#).unwrap_err();
    assert!(
...
fn figure_missing_output_errors() {
    let (s, _dir) = sb_with_workspace();
    let err = s.exec(r#"plot.figure({rows = 1, cols = 1})"#).unwrap_err();
    assert!(
        err.message.contains("output path is required"),
        "error: {}",
        err.message
...
fn radar_mismatched_data_errors() {
    let (s, _dir) = sb_with_workspace();
    let err = s
        .exec(r#"plot.radar({"A", "B", "C"}, {{10, 20}}, {output = "/workspace/bad.html"})"#)
        .unwrap_err();
    assert!(err.message.contains("indicators"), "error: {}", err.message);
}
...
fn histogram_html_has_plotly_and_histogram() {
    ...
    assert!(html.contains("plotly"), "HTML should reference Plotly.js");
    assert!(
        html.contains("histogram"),
        "HTML should contain histogram trace type"
    );
}
...
fn heatmap_html_has_plotly_and_heatmap() {
    ...
    assert!(
        ...
        "HTML should contain heatmap trace type"
    );
}
...
fn radar_html_has_plotly_and_scatterpolar() {
    ...
    assert!(html.contains("plotly"), "HTML should reference Plotly.js");
    assert!(
        html.contains("scatterpolar"),
        "HTML should contain scatterpolar trace type"
    );
}
...
fn candlestick_html_has_plotly_and_candlestick() {
    ...
    assert!(html.contains("plotly"), "HTML should reference Plotly.js");
    assert!(
        html.contains("candlestick"),
        "HTML should contain candlestick trace type"
    );
}
...
fn contour_html_has_plotly_and_contour() {
    ...
    assert!(html.contains("plotly"), "HTML should reference Plotly.js");
    assert!(
        html.contains("contour"),
        "HTML should contain contour trace type"
    );
}
...
fn table_html_has_plotly_and_table() {
    ...
    assert!(
        html.contains("table"),
        "HTML should contain table trace type"
    );
}
...
fn scatter3d_html_has_plotly_and_scatter3d() {
    ...
    assert!(html.contains("plotly"), "HTML should reference Plotly.js");
    assert!(
        html.contains("scatter3d"),
        "HTML should contain scatter3d trace type"
    );
}
...
fn surface_html_has_plotly_and_surface() {
    ...
    assert!(
        ...
        "HTML should contain surface trace type"
    );
}
...
fn sankey_html_has_plotly_and_sankey() {
    ...
    assert!(html.contains("plotly"), "HTML should reference Plotly.js");
    assert!(
        html.contains("sankey"),
        "HTML should contain sankey trace type"
    );
}
...
fn multi_html_has_plotly_and_traces() {
    ...
    assert!(
        html.contains("lines"),
        "HTML should contain lines mode for multi series"
    );
}
...
fn html_output_is_well_formed() {
    let (s, dir) = sb_with_workspace();
    s.exec(
        r#"plot.line({1,2,3}, {4,5,6}, {title="Well-Formed Test", output="/workspace/wf.html"})"#,
    )
    .unwrap();
    let html = std::fs::read_to_string(dir.path().join("wf.html")).unwrap();
    assert!(
        html.contains("<html") || html.contains("<!DOCTYPE"),
        "Should start with HTML doctype or tag"
    );
    assert!(
        html.contains("</html>") || html.contains("</script>"),
        "Should be a complete HTML document"
    );
    assert!(
        html.contains("Well-Formed Test"),
        "Should contain the chart title"
    );
    assert!(
        html.contains("Plotly") || html.contains("plotly"),
        "Should reference Plotly library"
    );
}
...
fn line_all_negative_values() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.line({1,2,3,4}, {-50,-30,-45,-10}, {output = "/workspace/neg.html"})"#)
        .unwrap();
    assert_eq!(r, "/workspace/neg.html");
...
fn line_zero_values() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.line({0,0,0}, {0,0,0}, {output = "/workspace/zero.html"})"#)
        .unwrap();
...
fn scatter_single_point() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.scatter({42}, {99}, {output="/workspace/ssp.html"})"#)
        .unwrap();
...
fn heatmap_single_cell() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.heatmap({{42}}, {output="/workspace/h1.html"})"#)
        .unwrap();
...
fn heatmap_negative_values() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.heatmap({{-5,-3},{-1,2}}, {output="/workspace/hn.html"})"#)
        .unwrap();
...
fn box_single_group() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.box({{1,2,3,4,5}}, {output="/workspace/b1.html"})"#)
        .unwrap();
...
fn contour_single_value() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.contour({0}, {0}, {{5}}, {output="/workspace/c1.html"})"#)
        .unwrap();
...
fn table_empty_rows() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.table({"A","B"}, {}, {output="/workspace/tempty.html"})"#)
        .unwrap();
...
fn scatter3d_single_point() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.scatter3d({1}, {2}, {3}, {output="/workspace/s3d1.html"})"#)
        .unwrap();
...
fn surface_single_cell() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.surface({{42}}, {output="/workspace/sf1.html"})"#)
        .unwrap();
...
fn pie_single_slice() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.pie({"Everything"}, {100}, {output="/workspace/ps.html"})"#)
        .unwrap();
...
fn radar_single_series() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.radar({"A","B","C"}, {{50,80,60}}, {output="/workspace/r1.html"})"#)
        .unwrap();
...
fn custom_dimensions_respected() {
    let (s, dir) = sb_with_workspace();
    s.exec(
        r#"plot.line({1,2,3}, {4,5,6}, {width=1200, height=400, output="/workspace/dim.html"})"#,
    )
    .unwrap();
...
fn minimum_dimensions_enforced() {
    ...
    s.exec(r#"plot.line({1,2,3}, {4,5,6}, {width=10, height=5, output="/workspace/small.html"})"#)
        .unwrap();
    let html = std::fs::read_to_string(dir.path().join("small.html")).unwrap();
    assert!(
        html.contains("plotly"),
        "HTML should still be valid Plotly output"
    );
}
...
fn all_themes_line() {
    ...
    for theme in &[
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_line.html", theme)),
            &format!("theme_{}", theme),
        );
    }
}
...
fn all_themes_scatter() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_scatter.html", theme)),
            &format!("scatter_{}", theme),
        );
    }
}
...
fn all_themes_histogram() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_hist.html", theme)),
            &format!("hist_{}", theme),
        );
    }
}
...
fn all_themes_heatmap() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_heat.html", theme)),
            &format!("heat_{}", theme),
        );
    }
}
...
fn all_themes_box() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_box.html", theme)),
            &format!("box_{}", theme),
        );
    }
}
...
fn all_themes_contour() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_cont.html", theme)),
            &format!("cont_{}", theme),
        );
    }
}
...
fn all_themes_scatter3d() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_s3d.html", theme)),
            &format!("s3d_{}", theme),
        );
    }
}
...
fn all_themes_surface() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_surf.html", theme)),
            &format!("surf_{}", theme),
        );
    }
}
...
fn all_themes_sankey() {
    ...
    for theme in ALL_THEMES {
        ...
        assert_plotly_html(
            &dir.path().join(format!("{}_sank.html", theme)),
            &format!("sank_{}", theme),
        );
    }
}
...
fn contour_mismatched_z_rows_errors() {
    ...
    let err = s
        ...
        .unwrap_err();
    assert!(err.message.contains("rows"), "error: {}", err.message);
}
...
fn contour_mismatched_z_cols_errors() {
    ...
    let err = s
        ...
        .unwrap_err();
    assert!(err.message.contains("columns"), "error: {}", err.message);
}
...
fn table_single_row() {
    let (s, dir) = sb_with_workspace();
    let r = s
        .exec(r#"return plot.table({"A", "B"}, {{"1", "2"}}, {output = "/workspace/tables.html"})"#)
        .unwrap();
...
fn scatter3d_mismatched_lengths_errors() {
    let (s, _dir) = sb_with_workspace();
    let err = s
        .exec(r#"plot.scatter3d({1, 2, 3}, {4, 5}, {7, 8, 9}, {output = "/workspace/bad.html"})"#)
        .unwrap_err();
    assert!(
        err.message.contains("same length"),
        "error: {}",
...
fn html_gallery_all_types() {
    ...
    let expected = vec![
        ...
        "gallery_box.html",
        "gallery_ohlc.html",
        "gallery_contour.html",
        "gallery_table.html",
        "gallery_scatter3d.html",
        "gallery_surface.html",
        "gallery_sankey.html",
    ];
