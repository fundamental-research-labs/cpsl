...
fn to_string_returns_non_empty() {
    let s = sb();
    let r = s.exec(r#"return qr.to_string("hello")"#).unwrap();
...
fn to_string_unicode_has_block_chars() {
    ...
    let has_blocks = r.contains('\u{2580}') || r.contains('\u{2584}') || r.contains('\u{2588}');
    assert!(
        has_blocks,
...
fn to_string_different_data_produces_different_output() {
    let s = sb();
    let r1 = s.exec(r#"return qr.to_string("hello")"#).unwrap();
    let r2 = s.exec(r#"return qr.to_string("world")"#).unwrap();
...
fn generate_svg_writes_file() {
    ...
    assert!(
        ...
        "File should contain SVG content, got: {}",
        &contents[..100.min(contents.len())]
    );
    assert!(
        contents.contains("rect"),
        "SVG should contain rect elements"
    );
}
...
fn generate_svg_with_custom_colors() {
    ...
    let contents = std::fs::read_to_string(&path).unwrap();
    assert!(
        contents.contains("rgb(255,0,0)"),
        "SVG should contain red color"
    );
    assert!(
        contents.contains("rgb(0,255,0)"),
        "SVG should contain green background"
    );
}
...
fn generate_short_aliases() {
    ...
    let contents = std::fs::read_to_string(&path).unwrap();
    assert!(
        contents.contains("rgb(0,0,255)"),
        "SVG should contain blue color from short alias"
    );
}
...
fn to_string_table_form_positional() {
    let s = sb();
    let r = s.exec(r#"return qr.to_string({[1]="hello"})"#).unwrap();
    assert!(
        !r.is_empty(),
        "Table-form to_string should return non-empty string"
    );
...
fn global_help_mentions_qr() {
    ...
    assert!(r.contains("qr"), "global help should list qr: {}", r);
}
