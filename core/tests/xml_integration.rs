...
fn xml_lua_help_uses_structured_params() {
    ...
    assert!(
        r.contains("xml.parse(text: string) -> table"),
        "Lua help should show structured params: {}",
        r
    );
    assert!(
        r.contains("xml.query(doc: table, path: string) -> table"),
        "Lua help should show multi-param: {}",
        r
    );
    assert!(
        r.contains("xml.encode(tree: table) -> string"),
        "Lua help should show encode: {}",
        r
    );
    assert!(
        r.contains("xml.text(node: table) -> string"),
        "Lua help should show text: {}",
        r
    );
}
...
fn xml_shell_help_uses_flag_syntax() {
    ...
    let luau = cpsl_core::sh_transpile::transpile_sh("xml")
        .unwrap()
        .luau_source;
    let r = s.exec(&luau).unwrap();
    assert!(
        r.contains("xml parse -t/--text <string>"),
        "shell help should use flag syntax: {}",
        r
    );
    assert!(
        r.contains("xml query -d/--doc <JSON> -p/--path <string>"),
        "shell help should show multi-param flags: {}",
        r
    );
    assert!(
        r.contains("xml encode -t/--tree <JSON>"),
        "shell help should show encode flags: {}",
        r
    );
}
...
fn parse_table_form() {
    ...
    let r = s
        .exec(
            r#"local doc = xml.parse({[1]="<root>hi</root>"}); return doc.tag .. " " .. doc.text"#,
        )
        .unwrap();
    assert_eq!(r, "root hi");
}
