...
fn global_help_includes_yfinance() {
    ...
    assert!(
        r.contains("yfinance"),
        "global help should list yfinance: {}",
        r
    );
}
...
fn history_long_ticker_errors() {
    ...
    let err = sb
        ...
        .unwrap_err();
    assert!(err.message.contains("too long"), "msg: {}", err.message);
}
...
fn history_denied_without_yahoo_domain() {
    let sb = sb_no_domains();
    let err = sb.exec(r#"yfinance.history("AAPL")"#).unwrap_err();
...
fn quote_denied_without_yahoo_domain() {
    let sb = sb_no_domains();
    let err = sb.exec(r#"yfinance.quote("AAPL")"#).unwrap_err();
    assert!(
        err.message.contains("denied") || err.message.contains("HTTP"),
        "expected domain denial: {}",
        err.message
...
fn search_denied_without_yahoo_domain() {
    let sb = sb_no_domains();
    let err = sb.exec(r#"yfinance.search("Apple")"#).unwrap_err();
    assert!(
        err.message.contains("denied") || err.message.contains("HTTP"),
        "expected domain denial: {}",
        err.message
...
fn quote_table_form_validates_ticker() {
    let sb = sb_no_domains();
    let err = sb.exec(r#"yfinance.quote({ticker=""})"#).unwrap_err();
    assert!(err.message.contains("empty"), "msg: {}", err.message);
}
...
fn shell_yfinance_help() {
    ...
    assert!(
        r.is_ok(),
        "shell yfinance help should not error: {:?}",
        r.err()
    );
}
