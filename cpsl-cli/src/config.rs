...
pub static MODULE_REGISTRY: &[ModuleManifest] = &[
    ModuleManifest { name: "fs",       description: "Filesystem operations (read, write, list, mkdir, etc.)", cargo_feature: "mod-fs" },
    ModuleManifest { name: "json",     description: "JSON encoding and decoding",                            cargo_feature: "mod-json" },
    ModuleManifest { name: "csv",      description: "CSV parsing and generation",                            cargo_feature: "mod-csv" },
    ModuleManifest { name: "yaml",     description: "YAML parsing and generation",                           cargo_feature: "mod-yaml" },
    ModuleManifest { name: "xml",      description: "XML parsing and generation",                            cargo_feature: "mod-xml" },
    ModuleManifest { name: "http",     description: "HTTP client (GET, POST, etc.) via sandboxed gateway",   cargo_feature: "mod-http" },
    ModuleManifest { name: "compress", description: "Archive and compression (zip, tar, gzip, bzip2, xz, 7z)", cargo_feature: "mod-compress" },
    ModuleManifest { name: "doc",      description: "Document parsing (PDF, Excel, RTF, Markdown)",          cargo_feature: "mod-doc" },
    ModuleManifest { name: "plot",     description: "Chart and plot generation (line, bar, scatter, histogram)", cargo_feature: "mod-plot" },
    ModuleManifest { name: "numx",     description: "Numerical computing (matrices, linear algebra, statistics)", cargo_feature: "mod-numpy" },
];
...
"#;
        ...
        assert!(err.contains("csv"), "error should mention module name, got: {}", err);
    }
...
"#;
    ...
    fn module_entry_display() {
        ...
        assert_eq!(
            format!("{}", ModuleEntry::External { source: "github.com/foo".into() }),
