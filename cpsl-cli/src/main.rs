...
fn resolve_language(bash: bool, python: bool, lua: bool) -> Language {
    if python {
        Language::Python
    } else if lua {
        Language::Lua
...
fn cmd_build(args: BuildArgs) {
    ...
    // Locate the cpsl workspace root (where cpsl-core and cpsl-cli live)
    let workspace_root = find_workspace_root().unwrap_or_else(|| {
        eprintln!("error: cannot find cpsl workspace root (looked for cpsl-core/Cargo.toml)");
        std::process::exit(1);
    });
...
/// Walk up from the executable to find the workspace root containing cpsl-core.
fn find_workspace_root() -> Option<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        for _ in 0..10 {
            if let Some(ref d) = dir {
                if d.join("cpsl-core").join("Cargo.toml").exists() {
                    return Some(d.clone());
                }
    ...
    for candidate in &[".", "stash/cpsl", "../cpsl", "../.."] {
        ...
        if p.join("cpsl-core").join("Cargo.toml").exists() {
...
fn cmd_run(args: RunArgs) {
    ...
    if !bin_path.exists() {
        ...
        eprintln!("Hint: build a sandbox first with `cpsl build -t {}`", args.name);
        std::process::exit(1);
    }
...
fn cmd_sandboxes() {
    ...
    for (name, modules, size, modified) in &entries {
        ...
        println!(
            "{:<nw$}  {:<40}  {:>10}  {}",
            name, modules_str, size_str, date_str,
            nw = name_width
        );
    }
}
    ...
    match lang {
        Language::Python => {
            let pyrt_source = find_pyrt().unwrap_or_else(|| {
                eprintln!("error: cannot find pyrt.luau — run from the cpsl directory");
                std::process::exit(1);
            });
            sandbox.setup_python_runtime(&pyrt_source).unwrap_or_else(|e| {
        ...
        Language::Bash => {
            let shrt_source = find_shrt().unwrap_or_else(|| {
                eprintln!("error: cannot find shrt.luau — run from the cpsl directory");
                std::process::exit(1);
            });
            sandbox.setup_shell_runtime(&shrt_source).unwrap_or_else(|e| {
...
/// Find pyrt.luau relative to the executable or CWD.
fn find_pyrt() -> Option<String> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        for _ in 0..5 {
            if let Some(ref d) = dir {
                let candidate = d.join("pyrt.luau");
                if candidate.exists() {
    ...
    // Try relative to CWD
    for candidate in &["pyrt.luau", "stash/cpsl/pyrt.luau"] {
...
/// Find shrt.luau relative to the executable or CWD.
fn find_shrt() -> Option<String> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        ...
        for _ in 0..5 {
            if let Some(ref d) = dir {
                let candidate = d.join("shrt.luau");
                if candidate.exists() {
                    return std::fs::read_to_string(&candidate).ok();
                }
                dir = d.parent().map(|p| p.to_path_buf());
    ...
    // Try relative to CWD
    for candidate in &["shrt.luau", "stash/cpsl/shrt.luau"] {
        if let Ok(content) = std::fs::read_to_string(candidate) {
            return Some(content);
        }
    }
    ...
    match transpile::transpile(python_source) {
        Ok(result) => {
            ...
            match sandbox.exec(&result.luau_source) {
                ...
                Err(e) => {
                    let translated = transpile::translate_error(
                        &e,
                        &result.source_map,
                        python_source,
                    );
