...
const JWT_ENCODE_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "algorithm", typ: "string", required: false, description: "JWT algorithm: HS256 (default), HS384, HS512" },
    FieldDoc { name: "expiresIn", typ: "number", required: false, description: "Expiration in seconds from now" },
];

const JWT_DECODE_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "algorithm", typ: "string", required: false, description: "JWT algorithm: HS256 (default), HS384, HS512" },
    FieldDoc { name: "validate", typ: "boolean", required: false, description: "Validate signature and expiry (default true)" },
];
...
pub fn register_crypto_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    crypto_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("sha256"), "crypto.sha256")?;
    ...
    crypto_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("sha512"), "crypto.sha512")?;
            let data = match &validated[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                _ => unreachable!(),
            };
    ...
    crypto_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("md5"), "crypto.md5")?;
            let data = match &validated[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                _ => unreachable!(),
            };
    ...
    crypto_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("hmac_sha256"), "crypto.hmac_sha256")?;
    ...
    crypto_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("hmac_sha512"), "crypto.hmac_sha512")?;
            let key = match &validated[0] {
                Value::String(s) => s.as_bytes().to_vec(),
                _ => unreachable!(),
            };
    ...
    crypto_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("encrypt"), "crypto.encrypt")?;
    ...
    crypto_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("decrypt"), "crypto.decrypt")?;
            ...
            let ciphertext = hex::decode(&ct_hex)
                .map_err(|e| mlua::Error::external(format!("crypto.decrypt: invalid ciphertext hex: {}", e)))?;
            let nonce_bytes = hex::decode(&nonce_hex)
                .map_err(|e| mlua::Error::external(format!("crypto.decrypt: invalid nonce hex: {}", e)))?;
            ...
            let plaintext = cipher
                ...
                .map_err(|e| mlua::Error::external(format!("crypto.decrypt: decryption failed (wrong key or corrupted data): {}", e)))?;
    ...
    crypto_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, CRYPTO_DOC.params("random_bytes"), "crypto.random_bytes")?;
