...
const CSV_WRITE_OPTS: &[FieldDoc] = &[
    FieldDoc { name: "delimiter", typ: "string", required: false, description: "Field delimiter (default \",\")" },
    FieldDoc { name: "headers", typ: "table", required: false, description: "Column headers (list of strings)" },
];

const CSV_PARSE_OPTS: &[FieldDoc] = &[
    FieldDoc { name: "delimiter", typ: "string", required: false, description: "Field delimiter (default \",\")" },
    FieldDoc { name: "quote", typ: "string", required: false, description: "Quote character (default '\"')" },
    FieldDoc { name: "header", typ: "boolean", required: false, description: "If true, first row is headers → returns keyed records" },
    FieldDoc { name: "skipRows", typ: "number", required: false, description: "Skip N rows before parsing" },
];
...
impl CsvOpts {
    ...
    fn apply_table(o: &mut Self, t: &mlua::Table) -> Result<(), mlua::Error> {
        // delimiter / d
        if let Ok(d) = t.get::<String>("delimiter").or_else(|_| t.get::<String>("d")) {
    ...
    csv_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            ...
            let (text, csv_opts) = match first {
                Value::String(s) => {
                    (s.as_bytes().to_vec(), CsvOpts::from_lua(&opts_arg)?)
                }
                Value::Table(t) => {
                    ...
                    let mut csv_opts = CsvOpts { delimiter: b',', quote: b'"', header: false, skip_rows: 0 };
                    CsvOpts::apply_table(&mut csv_opts, t)?;
                    (s.as_bytes().to_vec(), csv_opts)
                }
                _ => {
                    return Err(mlua::Error::external(
                        "csv.parse: argument 'text' expected string, got ".to_string() + first.type_name()
                    ));
                }
            };
        ...
        csv_table.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                ...
                let (path, csv_opts) = match first {
                    Value::String(s) => {
                        (s.to_string_lossy().to_string(), CsvOpts::from_lua(&opts_arg)?)
                    }
                    Value::Table(t) => {
                        let p: String = t.get::<String>(1).or_else(|_| t.get::<String>("path")).map_err(|_| {
                            mlua::Error::external(
                    ...
                    _ => {
                        return Err(mlua::Error::external(
                            "csv.parseFile: argument 'path' expected string, got ".to_string() + first.type_name()
                        ));
                    }
                };
        ...
        lua.create_function(|lua, args: MultiValue| {
            ...
            let first = match &args[0] {
                ...
                _ => return Err(mlua::Error::external(
                    "csv.stringify: argument 'rows' expected table, got ".to_string() + args[0].type_name()
                )),
            };
            ...
            if let Ok(inner) = maybe_inner {
                ...
                if maybe_nested.is_ok() || inner.raw_len() == 0 {
                    ...
                    let mut str_opts = StringifyOpts { delimiter: b',', headers: None };
            ...
            let str_opts = StringifyOpts { delimiter: b',', headers: None };
            let bytes = stringify_csv(&first, &str_opts)?;
            let s = String::from_utf8(bytes).map_err(mlua::Error::external)?;
            lua.create_string(&s)
        })?,
                ...
                move |_, args: MultiValue| {
                    ...
                    let (path, rows, str_opts) = match first {
                        Value::String(s) => {
                            ...
                            let r = rows_opt.ok_or_else(|| mlua::Error::external(
                        ...
                        Value::Table(t) => {
                            ...
                            let r: mlua::Table = t.get::<mlua::Table>(2).or_else(|_| t.get::<mlua::Table>("rows")).map_err(|_| {
                                mlua::Error::external(
                                    "csv.writeFile: missing required argument 'rows' (table)"
                                )
                            })?;
                            let mut so = StringifyOpts { delimiter: b',', headers: None };
                            StringifyOpts::apply_table(&mut so, t)?;
                            (p, r, so)
                        }
                        _ => {
                            return Err(mlua::Error::external(
                                "csv.writeFile: argument 'path' expected string, got ".to_string() + first.type_name()
                            ));
                        }
                    };
