...
use chrono::{
    Datelike, DateTime, Duration, NaiveDate, NaiveDateTime, TimeZone, Timelike, Utc, Weekday,
};
...
fn parse_datetime(s: &str) -> Result<DateTime<Utc>, String> {
    ...
    Err(format!("datetime.parse: could not parse '{}' — try providing an explicit format string", s))
}
...
pub fn register_datetime_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    dt_table.set(
        "timestamp",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, DATETIME_DOC.params("timestamp"), "datetime.timestamp")?;
            let dt_str = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    dt_table.set(
        "fromtimestamp",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, DATETIME_DOC.params("fromtimestamp"), "datetime.fromtimestamp")?;
    ...
    dt_table.set(
        "weekday",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, DATETIME_DOC.params("weekday"), "datetime.weekday")?;
            let dt_str = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    dt_table.set(
        "isoformat",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, DATETIME_DOC.params("isoformat"), "datetime.isoformat")?;
            let dt_str = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    dt_table.set(
        "strftime",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, DATETIME_DOC.params("strftime"), "datetime.strftime")?;
            let dt_str = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    dt_table.set(
        "strptime",
        lua.create_function(|_, args: MultiValue| {
            let validated = validate_args(&args, DATETIME_DOC.params("strptime"), "datetime.strptime")?;
            let s = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
