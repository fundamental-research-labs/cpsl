...
fn read_spreadsheet(data: &[u8], opts: &ReadOptions) -> Result<String, String> {
    ...
    let mut workbook =
        open_workbook_auto_from_rs(cursor).map_err(|e| format!("cannot open spreadsheet: {}", e))?;
    ...
    if let Some(sheet_num) = opts.sheet {
        ...
        let idx = sheet_num.checked_sub(1).ok_or("sheet number must be >= 1")?;
...
pub fn pdf_form_fields(
    ...
    for i in 0..pages.len() {
        let page = pages.get(i).map_err(|e| format!("cannot read page {}: {}", i, e))?;
        let annotations = page.annotations();
        for j in 0..annotations.len() {
            ...
            let value = match &form_field {
                ...
                PdfFormField::Checkbox(c) => {
                    ...
                    if checked { "true".to_string() } else { "false".to_string() }
                }
...
pub fn pdf_fill_form(
    ...
    let form = bindings.FPDFDOC_InitFormFillEnvironment(
        doc,
        Pin::as_mut(&mut ffi).get_mut(),
    );
    if form.is_null() {
    ...
    for i in 0..page_count {
        ...
        for j in 0..annot_count {
            ...
            let field_type =
                bindings.FPDFAnnot_GetFormFieldType(form, annot) as u32;
            ...
            match field_type {
                ...
                FF_CHECKBOX | FF_RADIOBUTTON => {
                    ...
                    let is_checked =
                        bindings.is_true(bindings.FPDFAnnot_IsChecked(form, annot));
        ...
        match op {
            ...
            PageOperation::Rotate(rotations) => {
                for &(idx, degrees) in rotations {
                    ...
                    let mut page = doc.pages().get(idx)
                        .map_err(|e| format!("cannot access page {}: {}", idx + 1, e))?;
...
pub fn pdf_add_annotation(
    ...
    let mut page = doc.pages().get(params.page)
    ...
    match params.annotation_type {
        ...
        AnnotationType::Stamp => {
            let mut annot = annotations.create_stamp_annotation()
...
pub fn pdf_watermark(
    ...
    let color = PdfColor::new(params.color.0, params.color.1, params.color.2, params.color.3);

    doc.pages().watermark(|group, index, width, height| {
        ...
        if params.rotation != 0.0 {
            ...
            let matrix = PdfMatrix::identity()
                .translate(PdfPoints::new(-center_x.value), PdfPoints::new(-center_y.value))?
...
fn read_rtf(data: &[u8]) -> Result<String, String> {
    let text = String::from_utf8(data.to_vec())
        .map_err(|e| format!("invalid RTF encoding: {}", e))?;
...
pub fn render_document_bytes(
    ...
    match (from, to) {
        ("html", "pdf") => {
            native_webview_pdf::html_to_pdf(text, &pdf_opts)
                .map_err(|e| format!("HTML→PDF: {}", e))
        }
        ("markdown", "pdf") | ("md", "pdf") => {
...
mod tests {
    ...
    fn test_format_detection() {
        assert_eq!(DocFormat::from_extension("report.xlsx"), Some(DocFormat::Xlsx));
        assert_eq!(DocFormat::from_extension("report.XLSX"), Some(DocFormat::Xlsx));
        ...
        assert_eq!(DocFormat::from_extension("slides.pptx"), Some(DocFormat::Pptx));
        ...
        assert_eq!(DocFormat::from_extension("photo.jpeg"), Some(DocFormat::Jpg));
        assert_eq!(DocFormat::from_extension("photo.JPEG"), Some(DocFormat::Jpg));
        assert_eq!(DocFormat::from_extension("photo.webp"), Some(DocFormat::Webp));
    ...
    fn test_image_format_local_extraction_fails() {
        ...
        for format in [DocFormat::Png, DocFormat::Jpg, DocFormat::Webp, DocFormat::Gif] {
    ...
    fn test_format_detection_html() {
        assert_eq!(DocFormat::from_extension("page.html"), Some(DocFormat::Html));
    ...
    fn test_convert_md_to_html() {
        let data = b"# Hello\n\nWorld";
        let result = convert_file(data, "md", "html", &ReadOptions::default(), &PageOptions::default()).unwrap();
    ...
    fn test_convert_html_to_txt() {
        ...
        let result = convert_file(data, "html", "txt", &ReadOptions::default(), &PageOptions::default()).unwrap();
    ...
    fn test_convert_csv_to_txt() {
        let data = b"a,b,c\n1,2,3\n";
        let result = convert_file(data, "csv", "txt", &ReadOptions::default(), &PageOptions::default()).unwrap();
        let text = String::from_utf8(result).unwrap();
    ...
    fn test_convert_unsupported_pair() {
        let data = b"hello";
        let err = convert_file(data, "csv", "pdf", &ReadOptions::default(), &PageOptions::default()).unwrap_err();
        assert!(err.contains("unsupported conversion"), "got: {}", err);
    }
