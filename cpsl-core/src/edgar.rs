...
fn sanitize_cik(cik: &str) -> Result<String, String> {
    ...
    let num_part = if trimmed.len() > 3
        && trimmed[..3].eq_ignore_ascii_case("cik")
    {
        trimmed[3..].trim()
    } else {
        trimmed
    ...
    if num_part.is_empty() || num_part.len() > 10 {
        return Err(format!(
            "edgar: CIK '{}' out of range (1-10 digits)",
            cik
        ));
    }
...
fn sanitize_form_type(form: &str) -> Result<String, String> {
    ...
    if trimmed.len() > 30 {
        return Err(format!("edgar: form type too long (max 30 chars): '{}'", form));
    }
...
fn edgar_get(gateway: &HttpGateway, url: &str) -> Result<serde_json::Value, mlua::Error> {
    ...
    let body_str = std::str::from_utf8(&response.body).map_err(|e| {
        mlua::Error::external(format!("edgar: invalid UTF-8 response: {}", e))
    })?;

    serde_json::from_str(body_str).map_err(|e| {
        mlua::Error::external(format!("edgar: invalid JSON response: {}", e))
    })
}
            ...
            if let Some(ciks) = source.get("ciks").and_then(|v| v.as_array()) {
                if let Some(cik) = ciks.first().and_then(|v| v.as_str()) {
                    ...
                    let url = format!(
                        ...
                        if cik_trimmed.is_empty() { "0" } else { cik_trimmed },
                        adsh_clean,
                        adsh
                    );
...
fn do_filings(
    ...
    let url = format!(
        "{}/CIK{}.json",
        EDGAR_SUBMISSIONS_BASE, cik_padded
    );
    ...
    let recent = json
        ...
        .ok_or_else(|| {
            mlua::Error::external("edgar: unexpected submissions response format (no filings.recent)")
        })?;
    ...
    let descriptions = recent.get("primaryDocDescription").and_then(|v| v.as_array());
    ...
    let cik_for_url = if cik_trimmed.is_empty() { "0" } else { cik_trimmed };
...
fn do_company(
    ...
    set_str(lua, &result, &json, "stateOfIncorporation", "stateOfIncorporation")?;
...
pub(crate) fn register_edgar_globals(
    ...
    // edgar.search(query, opts?)
    {
        ...
        edgar_table.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                let validated =
                    validate_args(&args, EDGAR_DOC.params("search"), "edgar.search")?;
    ...
    // edgar.filings(cik, opts?)
    {
        ...
        edgar_table.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                let validated =
                    validate_args(&args, EDGAR_DOC.params("filings"), "edgar.filings")?;
    ...
    // edgar.company(cik)
    {
        ...
        edgar_table.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                let validated =
                    validate_args(&args, EDGAR_DOC.params("company"), "edgar.company")?;
                let cik = match &validated[0] {
                    Value::String(s) => s.to_string_lossy().to_string(),
                    _ => {
                        return Err(mlua::Error::external(
