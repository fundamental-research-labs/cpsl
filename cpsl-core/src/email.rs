...
pub(crate) static EMAIL_DOC: ModuleDoc = ModuleDoc {
    ...
    functions: &[
        ...
        FnDoc {
            name: "parse",
            description:
                "Parse an email address. Returns {local, domain, full} or nil if invalid.",
            params: &[Param {
                name: "address",
                short: Some('a'),
                typ: ParamType::String,
...
pub fn register_email_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    email_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, EMAIL_DOC.params("isValid"), "email.isValid")?;
    ...
    email_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated =
                validate_args(&args, EMAIL_DOC.params("parse"), "email.parse")?;
            let address = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
    ...
    email_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, EMAIL_DOC.params("normalize"), "email.normalize")?;
            let address = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
