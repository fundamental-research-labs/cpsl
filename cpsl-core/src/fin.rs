    ...
    match v {
        Value::Table(t) => {
            ...
            if py_type.as_deref() == Some("list") {
                let data: mlua::Table = t.get("data").map_err(|_| {
                    mlua::Error::external("py.list missing 'data' field")
                })?;
    ...
    match vals.len() {
        ...
        1 => {
            if let Value::Table(t) = &vals[0] {
                ...
                let cashflows = table_to_vec(&cf_val)?;
                let finance_rate: f64 = t.get("finance_rate").or_else(|_| t.get("f")).map_err(|_| {
                    mlua::Error::external("fin.mirr: missing 'finance_rate'")
                })?;
                let reinvest_rate: f64 = t.get("reinvest_rate").or_else(|_| t.get("i")).map_err(|_| {
                    mlua::Error::external("fin.mirr: missing 'reinvest_rate'")
                })?;
                Ok((cashflows, finance_rate, reinvest_rate))
            } else {
                Err(mlua::Error::external("fin.mirr: expected (cashflows, finance_rate, reinvest_rate)"))
            }
        }
        _ => {
            ...
            let reinvest_rate = if vals.len() > 2 { to_f64(vals[2]) } else {
