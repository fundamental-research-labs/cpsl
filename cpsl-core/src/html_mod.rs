...
pub fn register_html_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    html_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated =
                validate_args(&args, HTML_DOC.params("parse"), "html.parse")?;
            ...
            let tostring_val =
                format!("HtmlDocument({} chars, \"{}...\")", source_len, preview);
            mt.set(
                "__tostring",
    ...
    html_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated =
                validate_args(&args, HTML_DOC.params("select"), "html.select")?;
    ...
    html_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated =
                validate_args(&args, HTML_DOC.params("select_one"), "html.select_one")?;
            let source = resolve_html_source(&validated[0])?;
            let selector_str = match &validated[1] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
    ...
    html_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, HTML_DOC.params("attr"), "html.attr")?;
