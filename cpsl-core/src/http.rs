...
const HTTP_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "headers", typ: "table", required: false, description: "Request headers {[name] = value}" },
    FieldDoc { name: "body", typ: "string", required: false, description: "Request body (string or JSON-encodable table)" },
];
    ...
    functions: &[
        FnDoc {
            ...
            params: &[
                ...
                Param { name: "opts", short: None, typ: ParamType::Table, required: false, fields: Some(HTTP_OPTS_FIELDS) },
            ],
            returns: ReturnType::Table,
            example: Some(r#"local resp = http.get("https://api.example.com/data", {headers = {Authorization = "Bearer tok"}})"#),
        },
        FnDoc {
            name: "post",
            ...
            returns: ReturnType::Table,
            example: Some(r#"http.post("https://api.example.com/items", {body = json.encode({name = "test"}), headers = {["Content-Type"] = "application/json"}})"#),
        },
        FnDoc {
            name: "put",
        ...
        FnDoc {
            ...
            params: &[
                Param { name: "url", short: None, typ: ParamType::String, required: true, fields: None },
                Param { name: "opts", short: None, typ: ParamType::Table, required: false, fields: Some(HTTP_OPTS_FIELDS) },
            ],
            returns: ReturnType::Table,
            example: None,
        ...
        FnDoc {
            ...
            params: &[
                Param { name: "method", short: Some('m'), typ: ParamType::String, required: true, fields: None },
                Param { name: "url", short: Some('u'), typ: ParamType::String, required: true, fields: None },
                Param { name: "opts", short: None, typ: ParamType::Table, required: false, fields: Some(HTTP_OPTS_FIELDS) },
            ],
            returns: ReturnType::Table,
            example: Some(r#"http.request({method="PATCH", url="https://api.example.com/items/1", body=json.encode({name="updated"}), headers={["Content-Type"]="application/json"}})"#),
        },
    ],
};

pub(crate) fn register_http_globals(lua: &Lua, gateway: Arc<HttpGateway>) -> Result<(), mlua::Error> {
            ...
            lua.create_function(move |lua, args: MultiValue| {
                ...
                let (url, opts) = match &args[0] {
                    mlua::Value::Table(ref t) => {
                        let url = t.get::<String>(1)
                            .map_err(|_| mlua::Error::external(format!(
                                "{}: missing required argument 'url' (string)", fn_name
                            )))?;
                        (url, Some(t.clone()))
                    }
                    ...
                    _ => return Err(mlua::Error::external(format!(
                        "{}: argument 'url' expected string, got {}", fn_name, args[0].type_name()
                    ))),
                };
            ...
            lua.create_function(
                move |lua, args: MultiValue| {
                    ...
                    let (method_str, url, opts) = match &args[0] {
                        ...
                        _ => return Err(mlua::Error::external(
                            "http.request: argument 'method' expected string, got ".to_string() + args[0].type_name()
                        )),
                    };
