...
const IMAGE_RESIZE_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "width", typ: "number", required: false, description: "Target width in pixels" },
    FieldDoc { name: "height", typ: "number", required: false, description: "Target height in pixels" },
    FieldDoc { name: "filter", typ: "string", required: false, description: "Resampling filter: nearest, bilinear, bicubic, lanczos3 (default)" },
];

const IMAGE_CROP_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "x", typ: "number", required: true, description: "Left edge (pixels from left)" },
    FieldDoc { name: "y", typ: "number", required: true, description: "Top edge (pixels from top)" },
    FieldDoc { name: "width", typ: "number", required: true, description: "Crop width in pixels" },
    FieldDoc { name: "height", typ: "number", required: true, description: "Crop height in pixels" },
];

const IMAGE_NEW_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "width", typ: "number", required: true, description: "Image width in pixels" },
    FieldDoc { name: "height", typ: "number", required: true, description: "Image height in pixels" },
    FieldDoc { name: "color", typ: "table", required: false, description: "{r, g, b} or {r, g, b, a} (default: white)" },
];

const IMAGE_TEXT_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "text", typ: "string", required: true, description: "Text to draw" },
    FieldDoc { name: "x", typ: "number", required: true, description: "X anchor position" },
    FieldDoc { name: "y", typ: "number", required: true, description: "Y anchor position" },
    FieldDoc { name: "size", typ: "number", required: true, description: "Font height in pixels" },
    FieldDoc { name: "color", typ: "table", required: true, description: "{r, g, b} or {r, g, b, a}" },
    FieldDoc { name: "font", typ: "string", required: false, description: "System font name or path to .ttf/.otf" },
    FieldDoc { name: "align", typ: "string", required: false, description: "left (default), center, right" },
    FieldDoc { name: "valign", typ: "string", required: false, description: "top (default), center, bottom" },
];
...
fn load_image(mounts: &MountTable, virtual_path: &str) -> Result<DynamicImage, mlua::Error> {
    let host_path = mounts.resolve_read(virtual_path).map_err(mlua::Error::external)?;
...
fn extract_string(args: &MultiValue, idx: usize, name: &str, func: &str) -> Result<String, mlua::Error> {
...
fn extract_number(args: &MultiValue, idx: usize, name: &str, func: &str) -> Result<f64, mlua::Error> {
    if args.len() > 1 || !matches!(args.get(0), Some(Value::Table(_))) {
        return match args.get(idx) {
...
pub fn register_image_globals(lua: &Lua, mounts: Arc<MountTable>) -> Result<(), mlua::Error> {
    ...
    // image.info(path) -> {width, height, format}
    {
        ...
        image_table.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                ...
                let fmt = ImageFormat::from_path(&host_path)
    ...
    // image.resize(input, output, {width?, height?, filter?})
    {
        ...
        image_table.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                let new_width: Option<u32> = opts.as_ref().and_then(|t| {
                    t.get::<f64>("width")
                        .ok()
                        .map(|n| n as u32)
                });
                let new_height: Option<u32> = opts.as_ref().and_then(|t| {
                    t.get::<f64>("height")
                        .ok()
                        .map(|n| n as u32)
                });
    ...
    // image.crop(input, output, {x, y, width, height})
    {
        ...
        image_table.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                let x: u32 = opts.get::<f64>("x").map_err(|_| {
                    mlua::Error::external("image.crop: opts.x is required (number)")
                })? as u32;
                let y: u32 = opts.get::<f64>("y").map_err(|_| {
                    mlua::Error::external("image.crop: opts.y is required (number)")
                })? as u32;
    ...
    // image.brightness(input, output, factor)
    {
        ...
        image_table.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                if args.is_empty() {
                    return Err(arg_error("image.brightness", IMAGE_DOC.params("brightness")));
                }
        ...
        image_table.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                let shapes_table = match if args.len() > 2 {
                    ...
                    _ => {
                        return Err(mlua::Error::external(
                            "image.draw: missing shapes list",
                        ))
                    }
                };
...
fn load_font(font_spec: Option<&str>) -> Result<FontArc, mlua::Error> {
    ...
    for pref in &preferred {
        ...
        if let Some(found) = fonts.iter().find(|f| {
            f.name.to_lowercase() == lower && f.style == "regular"
        }) {
