...
use crate::sandbox::{arg_error, validate_args, wrap_module_with_help_hints, FieldDoc, FnDoc, ModuleDoc, Param, ParamType, ReturnType};
use mlua::{Lua, MultiValue, Value};

const JSON_ENCODE_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "pretty", typ: "boolean", required: false, description: "Pretty-print with indentation (default false)" },
];
...
fn lua_to_json(value: &Value) -> Result<serde_json::Value, mlua::Error> {
    match value {
        ...
        Value::Number(n) => {
            let n = serde_json::Number::from_f64(*n).ok_or_else(|| {
                mlua::Error::external("cannot encode NaN or Infinity as JSON")
            })?;
