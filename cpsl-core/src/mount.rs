...
impl MountTable {
    ...
    pub fn is_synthetic_entry(&self, virtual_path: &str) -> bool {
        ...
        if let Some(slash_pos) = normalized.rfind('/') {
            let parent = if slash_pos == 0 { "/" } else { &normalized[..slash_pos] };
    ...
    pub fn resolve(&self, virtual_path: &str) -> Result<(PathBuf, MountPermission), MountError> {
        ...
        for (mount_virtual, entry) in &self.mounts {
            if is_under_mount(&normalized, mount_virtual) {
                if best_mount.is_none()
                    || mount_virtual.len() > best_mount.unwrap().0.len()
                {
        ...
        let (mount_virtual, entry) = best_mount
            .ok_or_else(|| MountError::NotFound(normalized.clone()))?;
        ...
        let canonical = if host_resolved.exists() {
            ...
            let parent = host_resolved.parent().ok_or_else(|| {
                MountError::PathTraversal(virtual_path.to_string())
            })?;
            if !parent.exists() {
                return Err(MountError::NotFound(virtual_path.to_string()));
            }
            let canonical_parent = parent.canonicalize()?;
            let file_name = host_resolved.file_name().ok_or_else(|| {
                MountError::PathTraversal(virtual_path.to_string())
            })?;
    ...
    pub fn resolve_write(&self, virtual_path: &str) -> Result<PathBuf, MountError> {
        if self.is_virtual_dir(virtual_path) && self.mounts.get(&normalize_virtual(virtual_path)).is_none() {
...
mod tests {
    ...
    fn test_list_root_shows_mount_children() {
        let dir = TempDir::new().unwrap();
        let mut table = MountTable::new();
        table.parse_and_add(&format!("{}:/data", dir.path().display())).unwrap();
        table.parse_and_add(&format!("{}:/workspace/src", dir.path().display())).unwrap();
    ...
    fn test_exists_virtual_dir() {
        let dir = TempDir::new().unwrap();
        let mut table = MountTable::new();
        table.parse_and_add(&format!("{}:/foo/bar", dir.path().display())).unwrap();
    ...
    fn test_root_mount_mkdir() {
        ...
        assert_eq!(
            path,
            root_dir.path().canonicalize().unwrap().join("test")
        );
    }
    ...
    fn test_root_mount_write_deep() {
        ...
        assert_eq!(
            path,
            root_dir.path().canonicalize().unwrap().join("tmp/scratch/file.txt")
        );
    }
    ...
    fn test_root_mount_specific_mount_wins() {
        ...
        assert_eq!(
            path,
            root_dir.path().canonicalize().unwrap().join("tmp")
        );
    }
