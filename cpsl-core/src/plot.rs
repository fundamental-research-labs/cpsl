...
use crate::sandbox::{arg_error, wrap_module_with_help_hints, FieldDoc, FnDoc, ModuleDoc, Param, ParamType, ReturnType};
...
use plotly::{Bar, BoxPlot, Candlestick, Contour, HeatMap, Histogram, Layout, Ohlc, Pie, Plot, Sankey, Scatter, Scatter3D, ScatterPolar, Surface, Table, Trace};
use std::sync::Arc;
...
const PLOT_SCATTER_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "colors", typ: "table", required: false, description: "List of hex color strings" },
    FieldDoc { name: "pointSize", typ: "number", required: false, description: "Point size in pixels" },
];
...
const PLOT_HISTOGRAM_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "width", typ: "number", required: false, description: "Width in pixels (default 800)" },
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "bins", typ: "number", required: false, description: "Number of bins (auto if omitted)" },
    FieldDoc { name: "colors", typ: "table", required: false, description: "List of hex color strings" },
];
...
const PLOT_PIE_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "colors", typ: "table", required: false, description: "List of hex color strings" },
    FieldDoc { name: "hole", typ: "number", required: false, description: "Donut hole size 0-1 (0 = pie, 0.4 = donut)" },
];
...
const PLOT_HEATMAP_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "width", typ: "number", required: false, description: "Width in pixels (default 800)" },
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "xlabels", typ: "table", required: false, description: "Custom X-axis labels" },
    FieldDoc { name: "ylabels", typ: "table", required: false, description: "Custom Y-axis labels" },
];

const PLOT_MULTI_SERIES_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "x", typ: "table", required: true, description: "X values" },
    FieldDoc { name: "y", typ: "table", required: true, description: "Y values" },
    FieldDoc { name: "type", typ: "string", required: false, description: r#""line" (default) or "scatter""# },
    FieldDoc { name: "label", typ: "string", required: false, description: "Legend label for this series" },
];
...
const PLOT_MULTI_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "colors", typ: "table", required: false, description: "List of hex color strings" },
    FieldDoc { name: "legend", typ: "boolean", required: false, description: "Show legend (default false)" },
    FieldDoc { name: "grid", typ: "boolean", required: false, description: "Show grid (default true)" },
];

const PLOT_FIGURE_OPTS_FIELDS: &[FieldDoc] = &[
    FieldDoc { name: "output", typ: "string", required: true, description: "Output file path (.html). REQUIRED." },
    FieldDoc { name: "rows", typ: "number", required: false, description: "Number of subplot rows (default 1)" },
    FieldDoc { name: "cols", typ: "number", required: false, description: "Number of subplot columns (default 1)" },
    FieldDoc { name: "width", typ: "number", required: false, description: "Width in pixels (default 800)" },
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
];
...
const PLOT_RADAR_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "colors", typ: "table", required: false, description: "List of hex color strings" },
    FieldDoc { name: "max", typ: "number", required: false, description: "Max value for all axes (default: auto from data)" },
    FieldDoc { name: "labels", typ: "table", required: false, description: "Series labels for legend" },
];
...
const PLOT_BOX_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "colors", typ: "table", required: false, description: "List of hex color strings" },
    FieldDoc { name: "horizontal", typ: "boolean", required: false, description: "Horizontal box plot (default false)" },
    FieldDoc { name: "points", typ: "string", required: false, description: "'all', 'outliers' (default), 'suspectedoutliers', or 'false'" },
    FieldDoc { name: "mean", typ: "boolean", required: false, description: "Show mean line (default false)" },
    FieldDoc { name: "labels", typ: "table", required: false, description: "Group labels for each data array" },
];
...
const PLOT_SCATTER3D_OPTS_FIELDS: &[FieldDoc] = &[
    ...
    FieldDoc { name: "height", typ: "number", required: false, description: "Height in pixels (default 600)" },
    FieldDoc { name: "colors", typ: "table", required: false, description: "List of hex color strings" },
    FieldDoc { name: "pointSize", typ: "number", required: false, description: "Point size in pixels (default 4)" },
];
...
fn extract_chart_opts(
    opts: &Option<mlua::Table>,
    fn_name: &str,
) -> Result<ChartOpts, mlua::Error> {
    let output = opts
        .as_ref()
        .and_then(|t| {
            t.get::<String>("output")
...
fn lua_table_to_string_matrix(val: &Value) -> Result<Vec<Vec<String>>, mlua::Error> {
    match val {
        ...
        _ => Err(mlua::Error::external("expected table of tables (string matrix)")),
    }
}
...
fn parse_sankey_links(links_tbl: &mlua::Table) -> Result<(Vec<usize>, Vec<usize>, Vec<f64>), mlua::Error> {
    ...
    for i in 1..=links_tbl.raw_len() {
        ...
        if src == 0 {
            return Err(mlua::Error::external(format!(
                "plot.sankey: links[{}] 'source' must be >= 1 (got 0)", i
            )));
        }
        if tgt == 0 {
            return Err(mlua::Error::external(format!(
                "plot.sankey: links[{}] 'target' must be >= 1 (got 0)", i
            )));
        }
        sources.push(src - 1);
        targets.push(tgt - 1);
...
fn render_bar(labels: &[String], values: &[f64], opts: &ChartOpts, horizontal: bool) -> String {
    ...
    if horizontal {
        ...
        let mut trace = Bar::new(labels.to_vec(), values.to_vec())
            .show_legend(false);
        if let Some(c) = opts.colors.first() {
            trace = trace.marker(Marker::new().color(c.clone()));
        }
...
fn render_histogram(data: &[f64], opts: &ChartOpts, bins: Option<usize>) -> String {
    let mut trace = Histogram::new(data.to_vec())
        .show_legend(false);
...
fn render_pie(labels: &[String], values: &[f64], opts: &ChartOpts, hole: Option<f64>) -> String {
    let mut trace = Pie::new(values.to_vec())
...
fn render_multi(series_list: &[SeriesData], opts: &ChartOpts) -> String {
    ...
    for (i, s) in series_list.iter().enumerate() {
        let name = s.label.clone().unwrap_or_else(|| format!("Series {}", i + 1));
...
fn render_figure(
    ...
    for (idx, sp) in subplots.iter().enumerate() {
        ...
        let x_axis_name = if axis_num == 1 { "x".to_string() } else { format!("x{}", axis_num) };
        let y_axis_name = if axis_num == 1 { "y".to_string() } else { format!("y{}", axis_num) };
        ...
        let trace: Box<dyn Trace> = match (&sp.chart_type[..], &sp.data) {
            ("line", SubplotData::XY(x, y)) => {
                Scatter::new(x.clone(), y.clone())
                    .mode(Mode::Lines)
                    .show_legend(false)
                    .x_axis(&x_axis_name)
                    .y_axis(&y_axis_name)
            }
            ("scatter", SubplotData::XY(x, y)) => {
                Scatter::new(x.clone(), y.clone())
                    .mode(Mode::Markers)
                    .show_legend(false)
                    .x_axis(&x_axis_name)
            ...
            ("bar", SubplotData::LabelValue(l, v)) => {
                Bar::new(l.clone(), v.clone())
                    .show_legend(false)
                    .x_axis(&x_axis_name)
                    .y_axis(&y_axis_name)
            }
...
fn render_box(
    ...
    let box_mean = if show_mean { BoxMean::True } else { BoxMean::False };

    for (i, data) in data_sets.iter().enumerate() {
        let name = group_labels.get(i)
            .cloned()
            .unwrap_or_else(|| format!("Group {}", i + 1));

        let mut trace = if horizontal {
            BoxPlot::new(data.clone()).name(name).orientation(Orientation::Horizontal)
        } else {
            BoxPlot::new(data.clone()).name(name)
        };

        trace = trace.box_points(box_points.clone()).box_mean(box_mean.clone());
...
fn render_scatter3d(
    ...
    let mut trace = Scatter3D::new(x.to_vec(), y.to_vec(), z.to_vec())
        .show_legend(false);
...
pub(crate) fn register_plot_globals(
    lua: &Lua,
                    ...
                    let (labels, values, horizontal, co) = match values_opt {
                        Some(values_val) => {
                            ...
                            let horizontal = opts
                                ...
                                .unwrap_or(false);
                            (labels, values, horizontal, extract_chart_opts(&opts, "plot.bar")?)
                        }
                        None => {
                            let t = match &first {
                                Value::Table(t) => t,
                            ...
                            let opts_tbl = Some(t.clone());
                            (labels, values, horizontal, extract_chart_opts(&opts_tbl, "plot.bar")?)
                        }
                    };
                ...
                move |_, args: MultiValue| {
                    ...
                    let (data, bins_opt, co) = match &first {
                        Value::Table(t) if has_output_key(t) => {
                            ...
                            let opts_tbl = Some(t.clone());
                            (data, bins_opt, extract_chart_opts(&opts_tbl, "plot.histogram")?)
                        }
                        _ => {
                ...
                move |_, args: MultiValue| {
                    ...
                    let (labels, values, hole, co) = match values_opt {
                        ...
                        None => {
                            ...
                            let opts_tbl = Some(t.clone());
                            (labels, values, hole, extract_chart_opts(&opts_tbl, "plot.pie")?)
                        }
                    };
                    if labels.len() != values.len() {
                        return Err(mlua::Error::external(
                ...
                move |_, args: MultiValue| {
                    ...
                    let (matrix, xlabels, ylabels, co) = match &first {
                        Value::Table(t) if has_output_key(t) => {
                            ...
                            let xlabels = t.get::<Value>("xlabels").ok()
                                .and_then(|v| lua_table_to_string_vec(&v).ok());
                            let ylabels = t.get::<Value>("ylabels").ok()
                                .and_then(|v| lua_table_to_string_vec(&v).ok());
                            let opts_tbl = Some(t.clone());
                            (matrix, xlabels, ylabels, extract_chart_opts(&opts_tbl, "plot.heatmap")?)
                        }
                        _ => {
                            let matrix = lua_table_to_f64_matrix(&first)?;
                            let xlabels = opts.as_ref().and_then(|t| {
                                t.get::<Value>("xlabels").ok()
                                    .and_then(|v| lua_table_to_string_vec(&v).ok())
                            });
                            let ylabels = opts.as_ref().and_then(|t| {
                                t.get::<Value>("ylabels").ok()
                                    .and_then(|v| lua_table_to_string_vec(&v).ok())
                            });
                            (matrix, xlabels, ylabels, extract_chart_opts(&opts, "plot.heatmap")?)
                        }
                    };
                    if matrix.is_empty() {
                        return Err(mlua::Error::external(
                            "heatmap matrix must not be empty",
                        ));
                    }
                ...
                move |_, args: MultiValue| {
                    ...
                    let (indicators, data, max_val, series_labels, co) = match data_opt {
                        Some(data_val) => {
                            ...
                            let labels = opts.as_ref()
                                .and_then(|t| t.get::<Value>("labels").ok())
                                .and_then(|v| lua_table_to_string_vec(&v).ok())
                                .unwrap_or_default();
                            (indicators, data, max_val, labels, extract_chart_opts(&opts, "plot.radar")?)
                        }
                        None => {
                            let t = match &first {
                                Value::Table(t) => t,
                            ...
                            let labels = t.get::<Value>("labels").ok()
                                .and_then(|v| lua_table_to_string_vec(&v).ok())
                                .unwrap_or_default();
                            let opts_tbl = Some(t.clone());
                            (indicators, data, max_val, labels, extract_chart_opts(&opts_tbl, "plot.radar")?)
                        }
                    };
                ...
                move |_, args: MultiValue| {
                    ...
                    let (data_sets, horizontal, points, show_mean, group_labels, co) = match &first {
                        Value::Table(t) if has_output_key(t) => {
                            ...
                            let horizontal = t.get::<bool>("horizontal").unwrap_or(false);
                            let points = t.get::<String>("points").unwrap_or_else(|_| "outliers".to_string());
                            let show_mean = t.get::<bool>("mean").unwrap_or(false);
                            let labels = t.get::<Value>("labels").ok()
                                .and_then(|v| lua_table_to_string_vec(&v).ok())
                                .unwrap_or_default();
                            let opts_tbl = Some(t.clone());
                            (data_sets, horizontal, points, show_mean, labels, extract_chart_opts(&opts_tbl, "plot.box")?)
                        }
                        _ => {
                            // Positional form: plot.box({{...},{...}}, {opts})
                            let data_sets = lua_table_to_f64_matrix(&first)?;
                            let horizontal = opts.as_ref().and_then(|t| t.get::<bool>("horizontal").ok()).unwrap_or(false);
                            let points = opts.as_ref().and_then(|t| t.get::<String>("points").ok()).unwrap_or_else(|| "outliers".to_string());
                            let show_mean = opts.as_ref().and_then(|t| t.get::<bool>("mean").ok()).unwrap_or(false);
                            let labels = opts.as_ref()
                                .and_then(|t| t.get::<Value>("labels").ok())
                                .and_then(|v| lua_table_to_string_vec(&v).ok())
                                .unwrap_or_default();
                            (data_sets, horizontal, points, show_mean, labels, extract_chart_opts(&opts, "plot.box")?)
                        }
                    };
                    ...
                    for (i, ds) in data_sets.iter().enumerate() {
                        if ds.is_empty() {
                            return Err(mlua::Error::external(format!(
                                "plot.box: data[{}] must not be empty", i + 1
                            )));
                        }
                    }
                    let html = render_box(&data_sets, &co, horizontal, &points, show_mean, &group_labels);
                    let host_path = ensure_output_dir(&m, &co.output)?;
                    std::fs::write(&host_path, &html).map_err(mlua::Error::external)?;
                    Ok(co.output)
                },
                ...
                move |_, args: MultiValue| {
                    ...
                    let (dates, open, close, low, high, co) = match open_opt {
                        Some(open_val) => {
                            ...
                            (dates, open, close, low, high, extract_chart_opts(&opts, "plot.ohlc")?)
                        }
                        None => {
                            let t = match &first {
                                Value::Table(t) => t,
                            ...
                            let high = lua_table_to_f64_vec(&hv)?;
                            let opts_tbl = Some(t.clone());
                            (dates, open, close, low, high, extract_chart_opts(&opts_tbl, "plot.ohlc")?)
                        }
                    };
                    let n = dates.len();
                    if open.len() != n || close.len() != n || low.len() != n || high.len() != n {
        ...
        plot.set(
            ...
            lua.create_function(
                move |_, args: MultiValue| {
                    ...
                    if z.is_empty() {
                        return Err(mlua::Error::external("plot.contour: z matrix must not be empty"));
                    }
                    if z.len() != y.len() {
                        return Err(mlua::Error::external(format!(
        ...
        plot.set(
            ...
            lua.create_function(
                move |_, args: MultiValue| {
                    ...
                    if headers.is_empty() {
                        return Err(mlua::Error::external("plot.table: headers must not be empty"));
                    }
                ...
                move |_, args: MultiValue| {
                    ...
                    let (x, y, z, point_size, co) = match y_opt {
                        ...
                        None => {
                            let t = match &first {
                                Value::Table(t) => t,
                                _ => return Err(mlua::Error::external("plot.scatter3d: expected table")),
                            };
                            let xv: Value = t.get("x")?;
                            let x = lua_table_to_f64_vec(&xv)?;
                            let yv: Value = t.get("y")?;
                            ...
                            let opts_tbl = Some(t.clone());
                            (x, y, z, ps, extract_chart_opts(&opts_tbl, "plot.scatter3d")?)
                        }
                    };
                ...
                move |_, args: MultiValue| {
                    ...
                    let (x, y, z, co) = match &first {
                        Value::Table(t) if has_output_key(t) => {
                            ...
                            let zv: Value = t.get("z")?;
                            let z = lua_table_to_f64_matrix(&zv)?;
                            let x = t.get::<Value>("x").ok()
                                .and_then(|v| lua_table_to_f64_vec(&v).ok());
                            let y = t.get::<Value>("y").ok()
                                .and_then(|v| lua_table_to_f64_vec(&v).ok());
                            let opts_tbl = Some(t.clone());
                        ...
                        _ => {
                            ...
                            if first_is_matrix {
                                ...
                                (Some(x), Some(y), z, extract_chart_opts(&opts, "plot.surface")?)
                            }
                        }
                    };
                    if z.is_empty() {
                        return Err(mlua::Error::external("plot.surface: z matrix must not be empty"));
                    }
                    ...
                    let (node_labels, sources, targets, values, co) = match links_opt {
                        Some(links_val) => {
                            ...
                            let links_tbl = match &links_val {
                                Value::Table(t) => unwrap_py_seq(t)?,
                                _ => return Err(mlua::Error::external("plot.sankey: links must be a table")),
                            };
                            ...
                            (nodes, sources, targets, values, extract_chart_opts(&opts, "plot.sankey")?)
                        }
                        None => {
                            let t = match &first {
                                Value::Table(t) => t,
                            ...
                            let (sources, targets, values) = parse_sankey_links(&links_tbl)?;
                            let opts_tbl = Some(t.clone());
                            (node_labels, sources, targets, values, extract_chart_opts(&opts_tbl, "plot.sankey")?)
                        }
                    };
                    if node_labels.is_empty() {
                        return Err(mlua::Error::external("plot.sankey: nodes must not be empty"));
                    }
