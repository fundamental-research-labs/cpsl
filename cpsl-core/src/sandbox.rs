    ...
    pub fn generated_signature(&self, mode: HelpMode) -> String {
        match mode {
            HelpMode::Lua => {
                ...
                let params_str: Vec<String> = self.params.iter().map(|p| {
                    if p.required {
            ...
            HelpMode::Shell => {
                ...
                let flags: Vec<String> = self.params.iter().map(|p| {
...
fn validate_table_form(
    ...
    for (i, param) in params.iter().enumerate() {
        ...
        let val: Value = t
            ...
            .or_else(|| t.get::<Value>(i + 1).ok().filter(|v| !matches!(v, Value::Nil)))
            .unwrap_or(Value::Nil);
...
impl Sandbox {
    ...
    pub fn exec(&self, code: &str) -> Result<String, ExecError> {
        ...
        let result: MultiValue = self.lua.load(code).set_name("input").eval().map_err(|e| clean_lua_error(&e))?;
    ...
    pub fn load_pyrt(&self, pyrt_source: &str) -> Result<(), SandboxError> {
        ...
        self.lua.load(&chunk).set_name("pyrt_loader").exec().map_err(SandboxError::Lua)?;
        Ok(())
    }
...
impl HostInfo {
    ...
    fn collect_macos() -> (String, u32, u64, String) {
        ...
        let cpu_brand = sysctl("machdep.cpu.brand_string")
        ...
        let mem_total = sysctl("hw.memsize")
            ...
            .unwrap_or(0);
        let os_version = sysctl("kern.osproductversion")
            .unwrap_or_default();
...
pub type VisionCallback =
    Arc<dyn Fn(&[u8], &str, &str) -> Result<String, String> + Send + Sync>;
...
impl SandboxBuilder {
    ...
    pub fn build(self) -> Result<Sandbox, SandboxError> {
        ...
        mount_table.add_synthetic_dir(
            "/etc",
            vec!["hostname".into(), "os-release".into()],
        );
        ...
        let tmpdir = if mount_table.mount_key_for("/tmp").is_none() {
            ...
            std::fs::create_dir_all(&dir).map_err(|e| SandboxError::Lua(mlua::Error::external(e)))?;
            // Canonicalize to resolve symlinks (e.g. /tmp → /private/tmp on macOS)
            let canonical = dir.canonicalize().map_err(|e| SandboxError::Lua(mlua::Error::external(e)))?;
        ...
        #[cfg(feature = "mod-fs")]
        register_fs_globals(&lua, mounts.clone(), synthetic_files, self.file_activity_callback.clone())?;
        ...
        if let (Some(ref store), Some(ref prompt)) = (&self.sfae_store, &self.sfae_prompt) {
            crate::sfae::register_sfae_globals(&lua, store.clone(), prompt.clone(), self.sfae_browser_opener.clone())?;
        }
        ...
        // Inject __sysinfo table so shell builtins (uname, hostname, etc.) can read real host values
        {
            ...
            sysinfo.set("sysname", "Sandbox").map_err(SandboxError::Lua)?;
            sysinfo.set("release", "1.0").map_err(SandboxError::Lua)?;
            sysinfo.set("machine", host_info.arch.as_str()).map_err(SandboxError::Lua)?;
            sysinfo.set("hostname", "sandbox").map_err(SandboxError::Lua)?;
            sysinfo.set("os", host_info.os.as_str()).map_err(SandboxError::Lua)?;
            sysinfo.set("os_version", host_info.os_version.as_str()).map_err(SandboxError::Lua)?;
            sysinfo.set("cpu_brand", host_info.cpu_brand.as_str()).map_err(SandboxError::Lua)?;
            sysinfo.set("cpu_cores", host_info.cpu_cores).map_err(SandboxError::Lua)?;
            sysinfo.set("mem_total_bytes", host_info.mem_total_bytes).map_err(SandboxError::Lua)?;
            lua.globals().set("__sysinfo", sysinfo).map_err(SandboxError::Lua)?;
        }
        ...
        lua.sandbox(true)?;
        Ok(Sandbox { lua, print_buf, needs_newline, _tmpdir: tmpdir })
    }
}
...
fn build_synthetic_files(info: &HostInfo) -> std::collections::HashMap<String, String> {
    ...
    map.insert(
        ...
        format!("Sandbox 1.0 ({} {} {})", info.os, info.os_version, info.arch),
    );
                    ...
                    return match path.as_str() {
                        "/dev/stdin" | "/dev/stdout" | "/dev/stderr" => {
                            Err(mlua::Error::external(format!(
                                "{}: not a regular file in sandbox", path
                            )))
                        }
                        _ if m.is_synthetic_entry(&path) => {
                            Err(mlua::Error::external(format!(
                                "{}: not readable in sandbox", path
                            )))
                        }
        ...
        fs.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                if m.is_mount_root(&src) {
                    return Err(mlua::Error::external(crate::MountError::MountRoot(
                        src,
                    )));
                }
    ...
    #[cfg(feature = "mod-grep")]
    {
        ...
        fs.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                ...
                let pattern: String = opts.get::<mlua::String>("pattern")
                    .map_err(|_| mlua::Error::external("fs.grep: missing required field 'pattern' (string)"))?
                    .to_string_lossy().to_string();
                let sandbox_path: String = opts.get::<mlua::String>("path")
                    .map_err(|_| mlua::Error::external("fs.grep: missing required field 'path' (string)"))?
                    .to_string_lossy().to_string();

                // Extract optional fields
                let glob_pattern: Option<String> = opts.get::<Value>("glob").ok().and_then(|v| match v {
                    Value::String(s) => Some(s.to_string_lossy().to_string()),
                    _ => None,
                });
                let max_count: Option<usize> = opts.get::<Value>("max_count").ok().and_then(|v| match v {
                    Value::Integer(n) => Some(n.max(0) as usize),
                    Value::Number(n) => Some((n as i64).max(0) as usize),
                    _ => None,
                });
                let files_only: bool = opts.get::<Value>("files_only").ok().and_then(|v| match v {
                ...
                let compiled_glob: Option<globset::GlobSet> = match glob_pattern {
                    Some(ref g) => {
                        let glob = globset::Glob::new(g)
                            .map_err(|e| mlua::Error::external(format!("fs.grep: invalid glob: {}", e)))?;
                ...
                let matcher = grep_regex::RegexMatcher::new(&pattern)
                    .map_err(|e| mlua::Error::external(format!("fs.grep: invalid pattern: {}", e)))?;

                // Resolve the sandbox path to host path
                let host_path = m.resolve_read(&sandbox_path).map_err(mlua::Error::external)?;
                ...
                if host_path.is_file() {
                    ...
                    return Err(mlua::Error::external(crate::MountError::NotFound(sandbox_path)));
                }
                ...
                'outer: for (file_host_path, file_virtual_path) in &files_to_search {
                    ...
                    let search_result = searcher.search_path(
                        ...
                        grep_searcher::sinks::UTF8(|line_num, line_content| {
                            ...
                            let line_str = line_content.trim_end_matches('\n').trim_end_matches('\r').to_string();
        ...
        fs.set(
            ...
            lua.create_function(move |_, args: MultiValue| {
                ...
                let sandbox_path: String = opts
                    ...
                    .map_err(|_| {
                        mlua::Error::external(
                            "fs.tree: missing required field 'path' (string)",
                        )
                    })?
                    .to_string_lossy()
                    .to_string();
                ...
                let glob_pattern: Option<String> = opts
                    .get::<Value>("glob")
                    .ok()
                    .and_then(|v| match v {
                        Value::String(s) => Some(s.to_string_lossy().to_string()),
                ...
                let mut output = format!("{}\n", if sandbox_path == "/" { "/".to_string() } else { format!("{}/", root_name) });
...
fn register_print(
    ...
    let print_fn = lua.create_function(move |_, values: MultiValue| {
        ...
        let Ok(mut nl) = needs_nl_print.lock() else { return Ok(()) };
        if *nl {
            b.push('\n');
        }
    ...
    let write_fn = lua.create_function(move |_, text: String| {
        let Ok(mut b) = buf2.lock() else { return Ok(()) };
        let Ok(mut nl) = needs_nl_write.lock() else { return Ok(()) };
        if *nl {
            b.push('\n');
        }
        b.push_str(&text);
...
mod tests {
    ...
    fn test_sandbox_blocks_dangerous_globals() {
        ...
        assert!(result.is_err(), "string.dump should be blocked in sandbox mode");
    }
    ...
    fn test_global_help_returns_help() {
        ...
        assert!(result.contains("Sandbox"), "should contain title: {}", result);
        assert!(result.contains("Modules:"), "should have modules section: {}", result);
        assert!(result.contains("print"), "should list print: {}", result);
        assert!(result.contains("help()"), "should list help itself: {}", result);
        assert!(
    ...
    fn test_global_help_works_without_return() {
        ...
        assert!(result.contains("Sandbox"), "bare help() should print: {}", result);
    }
    ...
    fn test_fs_help_works_without_return() {
        ...
        assert!(result.contains("fs — sandboxed filesystem"), "bare fs.help() should print: {}", result);
    }
    ...
    fn test_fs_runtime_error_no_hint() {
        ...
        let err = sandbox.exec("fs.read('/nonexistent')").unwrap_err().to_string();
        assert!(
            !err.contains("hint:"),
    ...
    fn test_clean_syntax_error() {
        let err = mlua::Error::SyntaxError {
            message: "[string \"input\"]:1: Expected 'end' (to close 'function' at line 1), got <eof>".to_string(),
    ...
    fn test_clean_fs_error_passthrough() {
        ...
        assert!(err.message.contains("No such file or directory"), "msg: {}", err.message);
    }
    ...
    fn test_unnamed_chunk_has_no_line_number() {
        ...
        let err = mlua::Error::RuntimeError(
            r#"[string ""]:5: some internal error"#.to_string(),
        );
        let e = clean_lua_error(&err);
    ...
    fn test_input_chunk_preserves_line_number() {
        ...
        let err = mlua::Error::RuntimeError(
            r#"[string "input"]:5: something went wrong"#.to_string(),
        );
        let e = clean_lua_error(&err);
    ...
    fn test_params() -> &'static [Param] {
        &[
            ...
            Param { name: "count", short: None, typ: ParamType::Number, required: false, fields: None },
        ]
    }
    ...
    fn test_validate_args_missing_required() {
        ...
        assert!(msg.contains("'text'"), "should name the missing param: {}", msg);
    }
    ...
    fn test_validate_args_no_args_mentions_all_required() {
        let params: &[Param] = &[
            Param { name: "doc", short: None, typ: ParamType::Table, required: true, fields: None },
            Param { name: "path", short: Some('p'), typ: ParamType::String, required: true, fields: None },
        ];
        let mv = MultiValue::new();
        ...
        assert!(msg.contains("xml.query"), "should mention function: {}", msg);
    }
        ...
        ModuleDoc {
            ...
            example: None,
                ...
                FnDoc {
                    ...
                    params: &[
                        Param { name: "table", short: None, typ: ParamType::Table, required: true, fields: None },
                        Param { name: "opts", short: None, typ: ParamType::Table, required: false, fields: None },
                    ],
                    returns: ReturnType::String,
            example: None,
                },
    ...
    fn test_format_help_lua_mode() {
        ...
        // Title
        assert!(help.contains("xml — XML parse, query & encode"), "title: {}", help);
        // Structured param rendering: module.fn(params) -> type
        assert!(help.contains("xml.parse(text: string) -> table"), "parse sig: {}", help);
        assert!(help.contains("xml.query(doc: table, path: string) -> table"), "query sig: {}", help);
        // Optional params have ?
        assert!(help.contains("xml.encode(table: table, opts?: table) -> string"), "encode sig: {}", help);
        // Help entry
    ...
    fn test_format_help_shell_mode() {
        ...
        assert!(help.contains("xml — XML parse, query & encode"), "title: {}", help);
        // Shell flag syntax
        assert!(help.contains("xml parse --text <string>"), "parse shell: {}", help);
        assert!(help.contains("xml query --doc <JSON> --path <string>"), "query shell: {}", help);
        ...
        assert!(!help.contains("xml.help()"), "shell help should not use Lua syntax: {}", help);
    }
    ...
    fn test_format_help_empty_params() {
        ...
        assert!(lua_help.contains("json.decode()"), "lua empty params: {}", lua_help);
        let shell_help = doc.format_help(HelpMode::Shell);
        assert!(shell_help.contains("json decode"), "shell empty params: {}", shell_help);
    }
    ...
    fn test_generated_signature_lua_required_params() {
        let f = FnDoc {
            name: "parse",
            description: "",
            params: &[Param { name: "text", short: None, typ: ParamType::String, required: true, fields: None }],
            returns: ReturnType::Table,
            example: None,
        };
        assert_eq!(f.generated_signature(HelpMode::Lua), "(text: string) -> table");
    }
    ...
    fn test_generated_signature_lua_optional_param() {
        let f = FnDoc {
            ...
            description: "",
            params: &[
                Param { name: "tree", short: None, typ: ParamType::Table, required: true, fields: None },
                Param { name: "opts", short: None, typ: ParamType::Table, required: false, fields: None },
            ],
            returns: ReturnType::String,
            example: None,
        };
        assert_eq!(f.generated_signature(HelpMode::Lua), "(tree: table, opts?: table) -> string");
    }
    ...
    fn test_generated_signature_lua_void_return() {
        ...
        assert_eq!(f.generated_signature(HelpMode::Lua), "(path: string, content: string)");
    }
    ...
    fn test_generated_signature_shell_optional_brackets() {
        let f = FnDoc {
            ...
            params: &[
                Param { name: "tree", short: None, typ: ParamType::Table, required: true, fields: None },
                Param { name: "indent", short: None, typ: ParamType::Boolean, required: false, fields: None },
            ],
            returns: ReturnType::String,
            example: None,
        };
    ...
    fn test_generated_signature_shell_with_short_flags() {
        let f = FnDoc {
            ...
            description: "",
            params: &[
                Param { name: "source", short: Some('s'), typ: ParamType::String, required: true, fields: None },
    ...
    fn test_generated_signature_shell_short_flag_optional() {
        let f = FnDoc {
            ...
            description: "",
            params: &[
                Param { name: "url", short: Some('u'), typ: ParamType::String, required: true, fields: None },
                Param { name: "headers", short: Some('H'), typ: ParamType::Table, required: false, fields: None },
            ],
            returns: ReturnType::Table,
            example: None,
        };
    ...
    fn test_generated_signature_lua_ignores_short_flags() {
        ...
        assert_eq!(f.generated_signature(HelpMode::Lua), "(path: string) -> string");
    }
    ...
    fn test_format_help_shell_mode_with_short_flags() {
        ...
        let help = doc.format_help(HelpMode::Shell);
        assert!(help.contains("compress zip -s/--source <string> -a/--archive <string>"), "shell short flags: {}", help);
    }
        ...
        for (mod_name, doc) in all_module_docs() {
            for f in doc.functions {
                ...
                assert_eq!(
                    ...
                    mod_name, f.name, lua_sig, sig_param_count, f.params.len()
                );
            }
        }
    }
