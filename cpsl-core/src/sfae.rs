    ...
    // sfae.credentials(domain) -> table of credential type strings
    {
        ...
        sfae.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                ...
                let types =
                    store::list_credential_types(&*guard, &domain, None).map_err(|e| {
                        mlua::Error::external(format!("sfae.credentials: {e}"))
                    })?;
    ...
    // sfae.prompt(domain, credential_type, opts?) -> true
    {
        ...
        sfae.set(
            ...
            lua.create_function(move |_lua, args: MultiValue| {
                ...
                let cred_type: CredentialType = cred_type_str.parse().map_err(|e: String| {
                    mlua::Error::external(format!("sfae.prompt: {e}"))
                })?;
    ...
    // sfae.request(method, url, opts?) -> {status, body, headers, ok}
    {
        ...
        sfae.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                ...
                let proxy_resp =
                    proxy::execute(&proxy_req, &*guard, &domain, None).map_err(|e| {
                        mlua::Error::external(format!("sfae.request: {e}"))
                    })?;
                drop(guard);
                ...
                {
                    try_refresh_and_retry(&store, &proxy_req, &domain)
                        .unwrap_or(proxy_resp)
                } else {
                    proxy_resp
                };
...
mod tests {
    ...
    impl BrowserOpener for CallbackSimulatingOpener {
        fn open_url(&self, url: &str) -> Result<(), String> {
            ...
            let state = test_extract_query_param(query, "state")
                .ok_or("missing state in auth URL")?;
    ...
    fn request_401_with_refresh_retries_successfully() {
        ...
        let store = Arc::new(Mutex::new(InMemoryStore::new()));
        {
            let mut guard = store.lock().unwrap();
            guard
                .set("127.0.0.1_ACCESS_TOKEN", "expired-tok")
                .unwrap();
            guard
