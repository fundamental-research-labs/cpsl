...
fn builtin_map(name: &str) -> Option<&'static str> {
    match name {
        "len" => Some("py.len"),
        "range" => Some("py.range"),
...
fn direct_method_map(method: &str) -> Option<&'static str> {
    match method {
...
fn is_passthrough_module(name: &str) -> bool {
    matches!(name, "fs" | "math" | "http" | "compress" | "json" | "csv" | "doc" | "sh" | "plot" | "yaml" | "xml" | "numx" | "random" | "fuzzy" | "phone" | "email" | "country" | "currency" | "datetime" | "image" | "base64" | "fin" | "yfinance" | "edgar" | "crypto" | "regex" | "html" | "url" | "qr")
}
...
fn python_module_to_luau(module: &str) -> String {
    match module {
        ...
        "rapidfuzz" | "rapidfuzz.fuzz" | "rapidfuzz.process" | "fuzzywuzzy" | "fuzzywuzzy.fuzz" | "fuzzywuzzy.process" => "fuzzy".to_string(),
        ...
        _ => format!(r#"error("module '{}' is not available in the sandbox. Run help() to see available modules.")"#, module),
    }
}

/// Map a Python `from X import Y` to its Luau equivalent expression.
/// E.g., `from matplotlib import pyplot` → `plot`
fn python_from_import_to_luau(module: &str, attr: &str) -> String {
    match (module, attr) {
...
impl<'a> Transpiler<'a> {
        ...
        match node {
            ...
            Expr::UnaryOp(u) => match u.op {
                ...
                UnaryOp::USub | UnaryOp::UAdd => {
                    ...
                    if t.is_num() { t } else { ExprType::Unknown }
                }
                _ => ExprType::Unknown,
            },
                    ...
                    match ident(&name.id) {
                        ...
                        "abs" | "min" | "max" | "sum" => {
                            ...
                            if let Some(first) = c.args.first() {
                                ...
                                if t.is_num() { t } else { ExprType::Int }
                            } else {
                                ExprType::Int
                            }
                        }
    ...
    fn expr(&mut self, node: &Expr) -> String {
        match node {
            ...
            Expr::GeneratorExp(g) => {
                ...
                let mut parts =
                    vec![format!("(function() local {} = py.list({{}}); ", tmp)];
    ...
    fn visit_compare(&mut self, node: &ExprCompare) -> String {
        ...
        for (op, comparator) in node.ops.iter().zip(node.comparators.iter()) {
            ...
            let can_raw_eq = (left_type == right_type || (left_type.is_num() && right_type.is_num()))
                && matches!(left_type, ExprType::Int | ExprType::Float | ExprType::Str | ExprType::Bool);
            let part = match op {
                CmpOp::Eq => {
                    if can_raw_eq {
                        format!("({} == {})", left, right)
    ...
    fn visit_call(&mut self, node: &ExprCall) -> String {
        ...
        if let Expr::Attribute(attr) = node.func.as_ref() {
            ...
            if let Expr::Name(name) = attr.value.as_ref() {
                ...
                let module_name = self.import_aliases.get(var_name).map(|s| s.as_str()).unwrap_or(var_name);
    ...
    fn emit_assignment(&mut self, target: &Expr, value: &Expr, range: TextRange) {
        match target {
            ...
            Expr::Attribute(attr) => {
                ...
                let val = self.expr(value);
                self.emit(
                    &format!("{}.{} = {}", obj, ident(&attr.attr), val),
                    range,
                );
            }
            _ => {
    ...
    fn has_complex_target(targets: &[Expr]) -> bool {
        targets
            .iter()
            .any(|t| matches!(t, Expr::Subscript(_) | Expr::Attribute(_) | Expr::Tuple(_) | Expr::List(_)))
    }
            ...
            if rhs_tuple.elts.len() == targets.len() {
                ...
                if has_complex {
                    ...
                    let names: Vec<String> =
                        targets.iter().map(|e| self.target_str(e)).collect();
    ...
    fn visit_augassign(&mut self, node: &StmtAugAssign) {
        ...
        let result_type = match node.op {
            Operator::Add => {
                if both_num {
                    if either_float { ExprType::Float } else { ExprType::Int }
            ...
            Operator::Div => {
                if both_num { ExprType::Float } else { ExprType::Unknown }
            }
            ...
            | Operator::BitXor | Operator::LShift | Operator::RShift => {
                if both_num { ExprType::Int } else { ExprType::Unknown }
            }
            _ => ExprType::Unknown,
        };
    ...
    fn visit_functiondef(&mut self, node: &StmtFunctionDef) {
        ...
        if !self.scopes.is_declared(name) {
            ...
            self.emit(
                &format!("function {}({})", name, param_str),
                node.range,
            );
        }
        ...
        for target in &node.targets {
            match target {
                Expr::Subscript(sub) => {
                    ...
                    let key = self.expr(&sub.slice);
                    self.emit(
                        &format!("py.setindex({}, {}, nil)", obj, key),
                        node.range,
                    );
                }
                Expr::Name(n) => {
...
mod tests {
    ...
    fn kwargs_module_call_only_kwargs() {
        ...
        assert!(
            result.contains("sh.ls({long = true})"),
            "got: {}",
            result
        );
    }
    ...
    fn kwargs_module_call_no_kwargs_unchanged() {
        ...
        assert!(
            result.contains(r#"fs.read("/path")"#),
            "got: {}",
            result
        );
    }
