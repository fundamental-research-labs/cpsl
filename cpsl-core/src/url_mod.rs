...
pub fn register_url_globals(lua: &Lua) -> Result<(), mlua::Error> {
    ...
    url_table.set(
        ...
        lua.create_function(|lua, args: MultiValue| {
            let validated =
                validate_args(&args, URL_DOC.params("query_parse"), "url.query_parse")?;
            let s = match &validated[0] {
                Value::String(s) => s.to_string_lossy().to_string(),
                _ => unreachable!(),
            };
            ...
            for pair_str in qs.split('&') {
                ...
                if let Some((k, v)) = pair_str.split_once('=') {
                    let key_decoded = percent_decode_str(k)
                        .decode_utf8_lossy()
                        .to_string();
                    let val_decoded = percent_decode_str(v)
                        .decode_utf8_lossy()
                        .to_string();
                    entry.set("key", lua.create_string(&key_decoded)?)?;
                    entry.set("value", lua.create_string(&val_decoded)?)?;
                } else {
                    let key_decoded = percent_decode_str(pair_str)
                        .decode_utf8_lossy()
                        .to_string();
                    entry.set("key", lua.create_string(&key_decoded)?)?;
    ...
    url_table.set(
        ...
        lua.create_function(|_, args: MultiValue| {
            let validated =
                validate_args(&args, URL_DOC.params("query_build"), "url.query_build")?;
