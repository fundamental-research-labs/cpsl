...
use crate::sandbox::{wrap_module_with_help_hints, FnDoc, ModuleDoc, Param, ParamType, validate_args, ReturnType};
...
fn parse_xml_tree(xml_text: &str) -> Result<XmlNode, mlua::Error> {
    ...
    if stack.len() != 1 {
        return Err(mlua::Error::external(
            "XML parse error: unclosed elements",
        ));
    }
...
fn write_node(writer: &mut Writer<Cursor<Vec<u8>>>, table: &mlua::Table) -> Result<(), mlua::Error> {
    ...
    let children: Option<mlua::Table> = table.get::<mlua::Table>("children").ok()
