...
pub(crate) static YAML_DOC: ModuleDoc = ModuleDoc {
    ...
    functions: &[
        ...
        FnDoc {
            name: "encode",
            description:
                "Serialize a value to a YAML string. Opts: {multiDoc?: boolean}",
            params: &[
                Param {
                    name: "value",
                    short: Some('v'),
        ...
        FnDoc {
            ...
            example: Some(r#"yaml.writeFile({path="/artifacts/config.yaml", value={host="localhost", port=8080}})"#),
        },
    ],
};
    ...
    // yaml.decodeFile(path) -> value
    {
        ...
        yaml_table.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                let validated = validate_args(&args, YAML_DOC.params("decodeFile"), "yaml.decodeFile")?;
                ...
                move |_, args: MultiValue| {
                    ...
                    let (path, value, multi_doc) = match first {
                        Value::String(s) => {
                            ...
                            let v = value_opt.ok_or_else(|| mlua::Error::external(
                        ...
                        Value::Table(t) => {
                            ...
                            let v: Value = t.get::<Value>(2)
                                .ok()
                                .filter(|v| !matches!(v, Value::Nil))
                                .or_else(|| t.get::<Value>("value").ok().filter(|v| !matches!(v, Value::Nil)))
                                .unwrap_or(Value::Nil);
                        ...
                        _ => {
                            return Err(mlua::Error::external(
                                "yaml.writeFile: argument 'path' expected string, got ".to_string() + first.type_name()
                            ));
                        }
                    };
                    ...
                    std::fs::write(&host_path, yaml_str.as_bytes())
