...
fn yf_get(gateway: &HttpGateway, url: &str) -> Result<serde_json::Value, mlua::Error> {
    ...
    headers.insert("User-Agent".to_string(), "Mozilla/5.0 (compatible; sandbox/1.0)".to_string());
    ...
    let body_str = std::str::from_utf8(&response.body).map_err(|e| {
        mlua::Error::external(format!("yfinance: invalid UTF-8 response: {}", e))
    })?;

    serde_json::from_str(body_str).map_err(|e| {
        mlua::Error::external(format!("yfinance: invalid JSON response: {}", e))
    })
}
...
fn parse_chart_response(lua: &Lua, json: &serde_json::Value) -> Result<mlua::Table, mlua::Error> {
    let result = json
        ...
        .and_then(|r| r.get(0))
        .ok_or_else(|| mlua::Error::external("yfinance: unexpected API response format (no chart.result[0])"))?;
...
fn do_quote(lua: &Lua, gateway: &HttpGateway, ticker: &str) -> Result<mlua::Table, mlua::Error> {
    ...
    let price = json
        ...
        .ok_or_else(|| mlua::Error::external("yfinance: unexpected API response format (no quoteSummary.result[0].price)"))?;
    ...
    set_yf_raw(lua, &result, price, "regularMarketChangePercent", "changePercent")?;
    ...
    set_yf_raw(lua, &result, price, "regularMarketPreviousClose", "previousClose")?;
...
fn do_info(lua: &Lua, gateway: &HttpGateway, ticker: &str) -> Result<mlua::Table, mlua::Error> {
    ...
    if let Some(summary) = modules.get("summaryDetail") {
        ...
        set_yf_raw(lua, &result, summary, "fiftyTwoWeekHigh", "fiftyTwoWeekHigh")?;
    ...
    if let Some(stats) = modules.get("defaultKeyStatistics") {
        ...
        set_yf_raw(lua, &result, stats, "earningsQuarterlyGrowth", "earningsGrowth")?;
        set_yf_raw(lua, &result, stats, "revenueQuarterlyGrowth", "revenueGrowth")?;
...
fn do_search(
    ...
    if query_trimmed.is_empty() {
        return Err(mlua::Error::external("yfinance.search: query cannot be empty"));
    }
...
pub(crate) fn register_yfinance_globals(
    ...
    // yfinance.info(ticker)
    {
        ...
        yf_table.set(
            ...
            lua.create_function(move |lua, args: MultiValue| {
                let validated =
                    validate_args(&args, YFINANCE_DOC.params("info"), "yfinance.info")?;
                let ticker = match &validated[0] {
                    Value::String(s) => s.to_string_lossy().to_string(),
                    _ => {
                        return Err(mlua::Error::external(
