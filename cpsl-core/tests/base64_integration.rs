...
fn decode_simple_string() {
    let s = sb();
    let r = s.exec(r#"return base64.decode("SGVsbG8sIFdvcmxkIQ==")"#).unwrap();
...
fn decode_with_whitespace() {
    ...
    let r = s
        .exec(r#"return base64.decode("SGVs\nbG8=")"#)
        .unwrap();
    assert_eq!(r, "Hello");
}
...
fn help_returns_help() {
    ...
    assert!(r.contains("base64 — Base64 encoding & decoding"), "help: {}", r);
...
fn nonexistent_fn_hint() {
    ...
    assert!(err.message.contains("base64.foo does not exist"), "msg: {}", err.message);
    assert!(err.message.contains("hint: call base64.help() for usage"), "msg: {}", err.message);
}
...
fn global_help_mentions_base64() {
    ...
    assert!(r.contains("base64"), "global help should list base64: {}", r);
}
