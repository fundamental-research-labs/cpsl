...
<Types xmlns="http://schemas.openxmlformats.org/package/2006/content-types">
  ...
  <Default Extension="xml" ContentType="application/xml"/>
    ...
    zip.start_file("ppt/slides/slide1.xml", zip::write::SimpleFileOptions::default())
        .unwrap();
    zip.write_all(br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"
  ...
  <p:cSld>
    ...
    zip.start_file("ppt/slides/slide2.xml", zip::write::SimpleFileOptions::default())
        .unwrap();
    zip.write_all(br#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<p:sld xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main"
       xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main">
...
fn doc_read_xlsx() {
    ...
    assert!(result.contains("TestSheet"), "should have sheet name: {}", result);
...
fn doc_read_rtf() {
    let dir = TempDir::new().unwrap();
    std::fs::write(
        dir.path().join("test.rtf"),
        r"{\rtf1 Hello RTF World}",
    )
    .unwrap();
    let sb = sandbox_with_dir(&dir);
...
fn doc_read_nonexistent_file() {
    ...
    let err = sb
        .exec(r#"doc.read("/data/missing.txt")"#)
        .unwrap_err();
    assert!(
...
fn doc_module_help() {
    ...
    assert!(result.contains("doc — document reading"), "help: {}", result);
...
fn doc_nonexistent_fn_hint() {
    ...
    assert!(err.message.contains("doc.foo does not exist"), "msg: {}", err.message);
    assert!(err.message.contains("hint: call doc.help() for usage"), "msg: {}", err.message);
}
...
fn global_help_mentions_doc_module() {
    ...
    assert!(result.contains("doc"), "help should mention doc: {}", result);
}
...
fn doc_read_html_extracts_text() {
    ...
    assert!(result.contains("Content here"), "should have content: {}", result);
}
...
fn doc_render_file_with_format_override() {
    ...
    assert!(
        output.contains("<h1>Override Test</h1>"),
        "got: {}",
        output
    );
}
...
fn doc_read_named_table() {
    ...
    let result = sb.exec(r#"return doc.read({path="/data/note.txt"})"#).unwrap();
...
fn doc_read_named_table_positional_key() {
    ...
    let result = sb.exec(r#"return doc.read({[1]="/data/note.txt"})"#).unwrap();
...
fn doc_read_explicit_vision_mode_on_txt() {
    ...
    let sb = sandbox_with_callback(&dir, None, cb);
    let result = sb.exec(r#"return doc.read("/data/data.txt", {mode="vision"})"#).unwrap();
...
fn doc_read_invalid_mode_errors() {
    ...
    let err = sb.exec(r#"doc.read("/data/test.txt", {mode="invalid"})"#).unwrap_err();
    assert!(
...
fn doc_help_without_callback_omits_mode() {
    let sb = Sandbox::new().unwrap();
    let result = sb.exec("return doc.help()").unwrap();
    assert!(!result.contains("mode"), "help without callback should not mention mode: {}", result);
    // Should still show structural-only description
    assert!(result.contains("structural"), "help should mention structural: {}", result);
}
...
fn doc_help_with_callback_shows_mode() {
    ...
    let sb = sandbox_with_callback(&dir, None, cb);
    let result = sb.exec("return doc.help()").unwrap();
    assert!(result.contains("mode"), "help with callback should show mode option: {}", result);
    assert!(result.contains("structural"), "help should mention structural: {}", result);
    assert!(result.contains("vision"), "help should mention vision: {}", result);
}
...
fn doc_read_async_exceeds_concurrency_limit() {
    ...
    let dir = TempDir::new().unwrap();
    for i in 0..12 {
        std::fs::write(dir.path().join(format!("img{}.png", i)), format!("data{}", i)).unwrap();
    }
...
mod pdfium_tests {
    ...
    fn pdfium_read_unicode() {
        ...
        assert!(
            !result.is_empty(),
            "should extract unicode text, got empty"
        );
    }
    ...
    fn pdfium_read_tables() {
        let sb = sandbox_with_pdfium();
        let result = sb
            .exec(r#"return doc.read("/pdf/tables.pdf")"#)
            .unwrap();
        assert!(
            !result.is_empty(),
            "should extract text from table PDF"
        );
    }
    ...
    fn pdfium_read_empty_pdf() {
        let sb = sandbox_with_pdfium();
        let result = sb
            .exec(r#"return doc.read("/pdf/empty.pdf")"#)
            .unwrap();
        assert!(
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "TestValue123", "filled value should be readable: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "0", "flattened PDF should have 0 form fields, got: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "4", "merged PDF should have 4 pages, got: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "5", "merged 3 PDFs should have 5 pages, got: {}", result);
    }
...
"#,
            ...
            .unwrap_err();
        assert!(
            err.message.contains("nonexistent") || err.message.contains("No such file") || err.message.contains("not found"),
            "should error on missing file: {}",
            err.message
        );
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "2,1", "split should produce 2-page and 1-page parts, got: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert!(
            result.contains("2|") && result.contains("split_1.pdf") && result.contains("split_2.pdf"),
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "1,1,1", "each split part should have 1 page, got: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "2", "should have 2 pages after deleting one, got: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "1", "should have 1 page after deleting two, got: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "3", "rotated PDF should still have 3 pages, got: {}", result);
    }
...
"#,
            ...
            .unwrap();
        assert_eq!(result, "3", "reordered PDF should still have 3 pages, got: {}", result);
    }
...
"#,
    ...
    fn help_includes_page_manipulation_functions() {
        let sb = sandbox_with_pdfium();
        let result = sb.exec("return doc.help()").unwrap();
        assert!(result.contains("mergePdf"), "help should mention mergePdf: {}", result);
        assert!(result.contains("splitPdf"), "help should mention splitPdf: {}", result);
        assert!(result.contains("editPages"), "help should mention editPages: {}", result);
    }
...
"#,
    ...
    fn help_includes_annotation_and_watermark_functions() {
        let sb = sandbox_with_pdfium();
        let result = sb.exec("return doc.help()").unwrap();
        assert!(result.contains("addAnnotation"), "help should mention addAnnotation: {}", result);
        assert!(result.contains("watermark"), "help should mention watermark: {}", result);
    }
}
