...
fn filings_path_traversal_cik_errors() {
    let sb = sb_no_domains();
    let err = sb
        .exec(r#"edgar.filings("../../etc/passwd")"#)
        .unwrap_err();
    assert!(
...
fn filings_too_long_cik_errors() {
    let sb = sb_no_domains();
    let err = sb
        .exec(r#"edgar.filings("12345678901")"#)
        .unwrap_err();
    assert!(
...
fn filings_cik_prefix_accepted() {
    ...
    let sb = sb_no_domains();
    let err = sb
        .exec(r#"edgar.filings("CIK0000320193")"#)
        .unwrap_err();
...
fn live_company_apple() {
    ...
    let r = sb
        ...
        .unwrap();
    assert!(
        r.contains("APPLE") || r.contains("Apple"),
        "got: {}",
        r
    );
