...
fn npv_basic() {
    ...
    assert_close(
        "return fin.npv(0.10, {-100, 30, 40, 50, 60})",
        38.877,
        0.01,
    );
}
...
fn npv_negative_result() {
    ...
    assert_close(
        "return fin.npv(0.10, {-1000, 100, 100})",
        -826.4463,
        0.01,
    );
}
...
fn mirr_simple() {
    ...
    assert_close(
        "return fin.mirr({-100, 200}, 0.05, 0.05)",
        1.0,
        0.0001,
    );
}
...
fn pmt_basic_loan() {
    ...
    assert_close(
        "return fin.pmt(0.075/12, 360, 200000)",
        -1398.43,
        0.1,
    );
}
...
fn pmt_beginning_of_period() {
    ...
    let r = s
        ...
        .unwrap();
    let val: f64 = r.parse().unwrap();
    assert!(val > 0.0, "beginning-of-period PMT should be smaller than end: diff={}", val);
}
...
fn rate_basic() {
    ...
    assert_close(
        "return fin.rate(360, -1398.43, 200000)",
        0.00625,
        0.0001,
    );
}
...
fn rate_with_fv() {
    ...
    assert_close(
        "return fin.rate(10, 0, -1000, 2000)",
        0.07177,
        0.001,
    );
}
...
fn irr_table_form() {
    assert_close(
        "return fin.irr({cashflows={-100, 110}})",
        0.1,
        0.0001,
    );
}
...
fn irr_non_convergence() {
    ...
    assert!(
        err.message.contains("converge") || err.message.contains("IRR") || err.message.contains("derivative"),
        "msg: {}",
        err.message
    );
}
...
fn fin_sandbox_no_network_access() {
    ...
    assert!(r.contains("-100.0000") && r.contains("500.0000"), "got: {}", r);
}
...
"#;
    ...
    let val: f64 = r.trim().parse().unwrap_or_else(|_| panic!("not a number: '{}'\ntranspiled:\n{}", r, transpiled.luau_source));
    assert!((val - 38.877).abs() < 0.1, "expected ~38.877, got: {}\ntranspiled:\n{}", val, transpiled.luau_source);
}
...
"#;
    let transpiled = transpile::transpile(py_code).unwrap();
    let r = s.exec(&transpiled.luau_source).unwrap();
    let val: f64 = r.trim().parse().unwrap_or_else(|_| panic!("not a number: {}", r));
    assert!((val - (-1398.43)).abs() < 1.0, "expected ~-1398.43, got: {}", val);
}
