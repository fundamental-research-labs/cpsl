...
fn write_file_read_back() {
    ...
    sb.exec("fs.write('/workspace/hello.txt', 'world')").unwrap();
...
fn mkdir_under_root_then_list() {
    ...
    let entries = sb
        .exec("local e = fs.list('/test'); return #e")
        .unwrap();
...
fn deep_mkdir_then_write_read() {
    ...
    sb.exec("fs.write('/workspace/a/b/c/file.txt', 'deep')").unwrap();
    let result = sb.exec("return fs.read('/workspace/a/b/c/file.txt')").unwrap();
...
fn exists_file() {
    ...
    assert_eq!(sb.exec("return fs.exists('/workspace/f.txt')").unwrap(), "true");
}
...
fn exists_directory() {
    ...
    assert_eq!(sb.exec("return fs.exists('/workspace/dir')").unwrap(), "true");
}
...
fn exists_nonexistent() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    assert_eq!(sb.exec("return fs.exists('/workspace/nope')").unwrap(), "false");
}
...
fn write_to_attachments_fails() {
    ...
    assert!(result.unwrap_err().to_string().contains("Read-only file system"));
}
...
fn workspace_mount_wins_over_root() {
    ...
    sb.exec("fs.write('/workspace/test.txt', 'from sandbox')").unwrap();
...
fn rename_workspace_root_fails() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("is a system directory"));
}
...
fn rename_within_workspace_works() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    sb.exec("fs.write('/workspace/old.txt', 'content')").unwrap();
    sb.exec("fs.rename('/workspace/old.txt', '/workspace/new.txt')").unwrap();
    let result = sb.exec("return fs.read('/workspace/new.txt')").unwrap();
    assert_eq!(result, "content");
    assert_eq!(sb.exec("return fs.exists('/workspace/old.txt')").unwrap(), "false");
}
...
fn rename_across_mounts_works() {
    ...
    sb.exec("fs.rename('/workspace/a.txt', '/artifacts/a.txt')").unwrap();
    let result = sb.exec("return fs.read('/artifacts/a.txt')").unwrap();
    assert_eq!(result, "data");
    assert_eq!(sb.exec("return fs.exists('/workspace/a.txt')").unwrap(), "false");
}
...
fn rename_within_same_mount_works() {
    ...
    sb.exec("fs.rename('/artifacts/old.txt', '/artifacts/new.txt')").unwrap();
...
fn isdir_real_dir() {
    ...
    assert_eq!(sb.exec("return fs.isdir('/workspace/mydir')").unwrap(), "true");
}
...
fn isdir_on_file() {
    ...
    assert_eq!(sb.exec("return fs.isdir('/workspace/file.txt')").unwrap(), "false");
}
...
fn isfile_on_file() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    sb.exec("fs.write('/workspace/file.txt', '')").unwrap();
    assert_eq!(sb.exec("return fs.isfile('/workspace/file.txt')").unwrap(), "true");
}
...
fn isfile_on_dir() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    sb.exec("fs.mkdir('/workspace/dir')").unwrap();
    assert_eq!(sb.exec("return fs.isfile('/workspace/dir')").unwrap(), "false");
}
...
fn isfile_on_nonexistent() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    assert_eq!(sb.exec("return fs.isfile('/workspace/nope')").unwrap(), "false");
}

#[test]
fn isdir_on_nonexistent() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    assert_eq!(sb.exec("return fs.isdir('/workspace/nope')").unwrap(), "false");
}
...
fn remove_file() {
    ...
    assert_eq!(sb.exec("return fs.exists('/workspace/bye.txt')").unwrap(), "false");
}
...
fn remove_directory_recursive() {
    ...
    sb.exec("fs.write('/workspace/dir/sub/file.txt', '')").unwrap();
    sb.exec("fs.remove('/workspace/dir')").unwrap();
    assert_eq!(sb.exec("return fs.exists('/workspace/dir')").unwrap(), "false");
}
...
fn rename_directory() {
    ...
    sb.exec("fs.write('/workspace/olddir/f.txt', 'hi')").unwrap();
    sb.exec("fs.rename('/workspace/olddir', '/workspace/newdir')").unwrap();
    let result = sb.exec("return fs.read('/workspace/newdir/f.txt')").unwrap();
    assert_eq!(result, "hi");
    assert_eq!(sb.exec("return fs.exists('/workspace/olddir')").unwrap(), "false");
}
...
fn copy_file_within_same_mount() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    sb.exec("fs.write('/workspace/src.txt', 'original')").unwrap();
    sb.exec("fs.copy('/workspace/src.txt', '/workspace/dst.txt')").unwrap();
    // Both files should exist with same content
    assert_eq!(sb.exec("return fs.read('/workspace/dst.txt')").unwrap(), "original");
    assert_eq!(sb.exec("return fs.read('/workspace/src.txt')").unwrap(), "original");
}

#[test]
fn copy_file_across_mounts() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    sb.exec("fs.write('/workspace/data.txt', 'cross-mount')").unwrap();
    sb.exec("fs.copy('/workspace/data.txt', '/artifacts/data.txt')").unwrap();
    assert_eq!(sb.exec("return fs.read('/artifacts/data.txt')").unwrap(), "cross-mount");
    // Source still exists
    assert_eq!(sb.exec("return fs.exists('/workspace/data.txt')").unwrap(), "true");
}
...
fn copy_from_readonly_source_works() {
    ...
    sb.exec("fs.copy('/attachments/doc.txt', '/artifacts/doc.txt')").unwrap();
    assert_eq!(sb.exec("return fs.read('/artifacts/doc.txt')").unwrap(), "readonly content");
}
...
fn copy_overwrites_existing_destination() {
    let (sb, _root, _ws, _att, _art) = standard_sandbox();
    sb.exec("fs.write('/workspace/src.txt', 'new content')").unwrap();
    sb.exec("fs.write('/workspace/dst.txt', 'old content')").unwrap();
    sb.exec("fs.copy('/workspace/src.txt', '/workspace/dst.txt')").unwrap();
    assert_eq!(sb.exec("return fs.read('/workspace/dst.txt')").unwrap(), "new content");
}
