...
fn test_grep_recursive_directory() {
    ...
    fs::write(dir.path().join("src/b.rs"), "fn helper() {}\nfn main() {}\n").unwrap();
...
fn test_fs_tree_dirs_only() {
    ...
    assert!(
        result.contains("3 directories, 0 files"),
        "got: {}",
        result
    );
}
...
fn test_fs_tree_empty_directory() {
    ...
    assert!(
        result.contains("0 directories, 0 files"),
        "got: {}",
        result
    );
}
...
fn test_fs_tree_single_file() {
    ...
    assert!(
        result.contains("0 directories, 1 file"),
        "got: {}",
        result
    );
}
...
fn test_fs_tree_connectors_correct() {
    ...
    let sandbox = sandbox_with_dir(&dir, "/data", "rw");
    let result = sandbox
        .exec(r#"return fs.tree({path="/data"})"#)
        .unwrap();
...
fn test_fs_tree_root_shows_all_mounts() {
    ...
    assert!(result.contains("workspace/"), "should show workspace mount: {}", result);
    assert!(result.contains("attachments/"), "should show attachments mount: {}", result);
    assert!(result.contains("a.txt"), "should show files inside workspace: {}", result);
    assert!(result.contains("b.txt"), "should show files inside attachments: {}", result);
    // Virtual dirs like tmp, home should appear
    assert!(result.contains("tmp/"), "should show tmp virtual dir: {}", result);
}
...
fn test_fs_tree_root_depth_1_shows_top_level_only() {
    ...
    assert!(result.contains("workspace/"), "should show workspace: {}", result);
    assert!(!result.contains("file.txt"), "depth=1 should not show files inside mounts: {}", result);
}
...
fn test_fs_tree_virtual_dirs_shown() {
    ...
    assert!(result.contains("dev/"), "should show /dev synthetic dir: {}", result);
    assert!(result.contains("proc/"), "should show /proc synthetic dir: {}", result);
    assert!(result.contains("etc/"), "should show /etc synthetic dir: {}", result);
}
...
fn test_shell_tree_command() {
    ...
    assert!(result.contains("workspace"), "should show root name: {}", result);
    assert!(result.contains("hello.txt"), "should show files: {}", result);
    assert!(result.contains("subdir/"), "should show dirs: {}", result);
    assert!(result.contains("inner.txt"), "should show nested files: {}", result);
}
...
fn test_shell_tree_with_depth_flag() {
    ...
    assert!(!result.contains("deep.txt"), "depth=1 should hide nested file: {}", result);
}
...
fn test_shell_tree_bare_shows_cwd() {
    ...
    assert!(result.starts_with("/\n"), "bare tree should show / as root, got: {}", result);
    assert!(result.contains("workspace/"), "should show workspace mount: {}", result);
