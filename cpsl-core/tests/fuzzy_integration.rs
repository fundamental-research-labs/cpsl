...
fn ratio_identical_strings() {
    let s = sb();
    let r = s
        .exec(r#"return fuzzy.ratio("hello", "hello")"#)
        .unwrap();
    assert_eq!(r, "100");
}
...
fn ratio_completely_different() {
    let s = sb();
    let r = s
        .exec(r#"return fuzzy.ratio("abc", "xyz")"#)
        .unwrap();
...
fn partial_ratio_substring_match() {
    ...
    let r = s
        ...
        .unwrap();
    let score: f64 = r.parse().unwrap();
    assert_eq!(score, 100.0, "exact substring should score 100, got {}", score);
}
...
fn token_sort_ratio_reordered() {
    let s = sb();
    let r = s
        .exec(r#"return fuzzy.token_sort_ratio("fuzzy wuzzy was a bear", "wuzzy fuzzy was a bear")"#)
        .unwrap();
    let score: f64 = r.parse().unwrap();
    assert_eq!(score, 100.0, "reordered tokens should score 100, got {}", score);
}
...
fn token_set_ratio_subset() {
    ...
    let r = s
        ...
        .unwrap();
    let score: f64 = r.parse().unwrap();
    assert!(score > 90.0, "near-duplicate with extra token should score high, got {}", score);
}
...
fn extract_basic() {
    ...
    assert!(r.contains("true"), "expected high score for best match, got: {}", r);
}
...
fn distance_one_empty() {
    let s = sb();
    let r = s
        .exec(r#"return fuzzy.distance("hello", "")"#)
        .unwrap();
...
fn ratio_unicode_strings() {
    let s = sb();
    let r = s
        .exec(r#"return fuzzy.ratio("café", "cafe")"#)
        .unwrap();
    let score: f64 = r.parse().unwrap();
    assert!(score > 70.0, "Unicode near-match should score reasonably, got {}", score);
}
...
fn distance_unicode_strings() {
    let s = sb();
    let r = s
        .exec(r#"return fuzzy.distance("café", "cafe")"#)
        .unwrap();
...
fn extract_preserves_original_indices() {
    ...
    assert!(r.starts_with("2"), "best match should be index 2, got: {}", r);
}
