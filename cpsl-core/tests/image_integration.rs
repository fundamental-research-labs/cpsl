...
fn resize_returns_output_path() {
    ...
    let r = s
        .exec(
            r#"return image.resize("/workspace/src.png", "/workspace/out.png", {width = 50})"#,
        )
        .unwrap();
...
fn grayscale_table_form() {
    ...
    let r = s
        .exec(
            r#"return image.grayscale({[1]="/workspace/src.png", [2]="/workspace/gray.png"})"#,
        )
        .unwrap();
    assert_eq!(r, "/workspace/gray.png");
...
fn cannot_read_outside_mount() {
    ...
    assert!(
        err.message.contains("No such file") || err.message.contains("not found") || err.message.contains("failed"),
...
fn cannot_write_outside_mount() {
    ...
    let err = s
        ...
        .unwrap_err();
    assert!(
        err.message.contains("No such file") || err.message.contains("Read-only") || err.message.contains("denied"),
...
fn cannot_traverse_path() {
    ...
    let err = s
        ...
        .unwrap_err();
    assert!(
        err.message.contains("traversal") || err.message.contains("denied") || err.message.contains("No such"),
...
fn shell_image_grayscale() {
    ...
    let luau = result.unwrap().luau_source;
    let r = s.exec(&luau);
    assert!(r.is_ok(), "shell image grayscale should not error: {:?}", r.err());
    assert!(dir.path().join("gray.png").exists());
}
...
fn overwrite_existing_file() {
    ...
    // First write
    s.exec(r#"image.resize("/workspace/src.png", "/workspace/out.png", {width = 50, height = 50})"#)
        .unwrap();

    // Overwrite
    s.exec(r#"image.resize("/workspace/src.png", "/workspace/out.png", {width = 25, height = 25})"#)
        .unwrap();
...
fn new_creates_blank_image() {
    ...
    // Verify pixel color
    let img = image::open(dir.path().join("blank.png")).unwrap().to_rgba8();
...
fn create_background_then_composite() {
    ...
    let img = image::open(dir.path().join("result.png")).unwrap().to_rgba8();
...
fn fonts_entries_have_name_path_style() {
    ...
    let r = s
        ...
        .unwrap();
    assert!(r.contains("|"), "expected name|style|path_check, got: {}", r);
...
fn blur_applies_gaussian() {
    ...
    let blurred = image::open(dir.path().join("blurred.png")).unwrap().to_rgba8();
...
fn rotate_exact_45_degrees() {
    ...
    let corner = img.get_pixel(0, 0);
    assert_eq!(corner.0[3], 0, "corner should be transparent after 45° rotation");
...
fn pil_imagedraw_import_maps_to_image() {
    ...
    let transpiled = transpile::transpile(py_code).unwrap();
    assert!(
        transpiled.luau_source.contains("image") || transpiled.luau_source.is_empty()
...
"#;
    ...
    assert!(
        transpiled.luau_source.contains("Image.draw") || transpiled.luau_source.contains("image.draw"),
...
fn help_includes_phase3_functions() {
    ...
    assert!(r.contains("image.fonts"), "help should include fonts: {}", r);
    assert!(r.contains("image.blur"), "help should include blur: {}", r);
    assert!(r.contains("image.sharpen"), "help should include sharpen: {}", r);
    assert!(
...
fn text_invalid_valign_errors() {
    ...
    let err = r.unwrap_err().to_string();
    assert!(err.contains("valign"), "error should mention valign: {}", err);
}
...
fn measure_text_returns_dimensions() {
    ...
    assert!(line_height > 0.0, "line_height should be > 0: {}", line_height);
}
