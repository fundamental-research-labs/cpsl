...
fn linalg_det_2x2() {
    ...
    assert!((val - (-2.0)).abs() < 1e-10, "det should be -2, got {}", val);
}
...
fn shape_mismatch_add() {
    ...
    let err = s.exec(r#"
        ...
        numx.add(a, b)
    "#).unwrap_err();
    assert!(err.message.contains("shape mismatch"), "msg: {}", err.message);
}
...
fn reshape_size_mismatch() {
    ...
    let err = s.exec(r#"
        ...
        numx.reshape(a, {2, 2})
    "#).unwrap_err();
    assert!(err.message.contains("cannot reshape"), "msg: {}", err.message);
}
...
"#;
    ...
    assert!((val - 1.0).abs() < 1e-10, "det of identity should be 1, got {}", val);
}
