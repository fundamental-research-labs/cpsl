...
fn xml_parse_no_args_mentions_params() {
    ...
    assert!(msg.contains("text"), "xml.parse error should mention 'text', got: {msg}");
}

#[test]
fn xml_parse_file_no_args_mentions_params() {
    let msg = exec_err(&sb(), "xml.parseFile()");
    assert!(msg.contains("path"), "xml.parseFile error should mention 'path', got: {msg}");
}

#[test]
fn xml_query_no_args_mentions_params() {
    let msg = exec_err(&sb(), "xml.query()");
    assert!(msg.contains("doc") && msg.contains("path"),
...
fn xml_text_no_args_mentions_params() {
    ...
    assert!(msg.contains("node"), "xml.text error should mention 'node', got: {msg}");
}

#[test]
fn xml_encode_no_args_mentions_params() {
    let msg = exec_err(&sb(), "xml.encode()");
    assert!(msg.contains("tree"), "xml.encode error should mention 'tree', got: {msg}");
}
...
fn json_decode_no_args_mentions_params() {
    ...
    assert!(msg.contains("text"), "json.decode error should mention 'text', got: {msg}");
}

#[test]
fn json_encode_no_args_mentions_params() {
    let msg = exec_err(&sb(), "json.encode()");
    assert!(msg.contains("value"), "json.encode error should mention 'value', got: {msg}");
}
...
fn csv_parse_no_args_mentions_params() {
    ...
    assert!(msg.contains("text"), "csv.parse error should mention 'text', got: {msg}");
}

#[test]
fn csv_stringify_no_args_mentions_params() {
    let msg = exec_err(&sb(), "csv.stringify()");
    assert!(msg.contains("rows"), "csv.stringify error should mention 'rows', got: {msg}");
}
...
fn yaml_decode_no_args_mentions_params() {
    ...
    assert!(msg.contains("text"), "yaml.decode error should mention 'text', got: {msg}");
}

#[test]
fn yaml_encode_no_args_mentions_params() {
    let msg = exec_err(&sb(), "yaml.encode()");
    assert!(msg.contains("value"), "yaml.encode error should mention 'value', got: {msg}");
}
...
fn fs_read_no_args_mentions_params() {
    ...
    assert!(msg.contains("path"), "fs.read error should mention 'path', got: {msg}");
}

#[test]
fn fs_write_no_args_mentions_params() {
    let msg = exec_err(&sb(), "fs.write()");
    assert!(msg.contains("path") && msg.contains("content"),
...
fn fs_list_no_args_mentions_params() {
    ...
    assert!(msg.contains("path"), "fs.list error should mention 'path', got: {msg}");
}

#[test]
fn fs_mkdir_no_args_mentions_params() {
    let msg = exec_err(&sb(), "fs.mkdir()");
    assert!(msg.contains("path"), "fs.mkdir error should mention 'path', got: {msg}");
}

#[test]
fn fs_remove_no_args_mentions_params() {
    let msg = exec_err(&sb(), "fs.remove()");
    assert!(msg.contains("path"), "fs.remove error should mention 'path', got: {msg}");
}

#[test]
fn fs_copy_no_args_mentions_params() {
    let msg = exec_err(&sb(), "fs.copy()");
    assert!(msg.contains("src") && msg.contains("dst"),
...
fn compress_unzip_no_args_mentions_params() {
    ...
    assert!(msg.contains("archive") && msg.contains("dest"),
...
fn compress_zip_no_args_mentions_params() {
    ...
    assert!(msg.contains("source") && msg.contains("archive"),
...
fn compress_bzip2_no_args_mentions_params() {
    ...
    assert!(msg.contains("input") && msg.contains("output"),
...
fn doc_read_no_args_mentions_params() {
    ...
    assert!(msg.contains("path"), "doc.read error should mention 'path', got: {msg}");
}

#[test]
fn doc_render_no_args_mentions_params() {
    let msg = exec_err(&sb(), "doc.render()");
    assert!(msg.contains("text") && msg.contains("from") && msg.contains("to"),
...
fn doc_render_file_no_args_mentions_params() {
    ...
    assert!(msg.contains("source") && msg.contains("target"),
...
fn plot_bar_no_args_mentions_params() {
    ...
    assert!(msg.contains("labels") && msg.contains("values"),
...
fn xml_shell_help_lists_flag_params() {
    ...
    assert!(out.contains("--text"), "xml help(shell) should list --text, got: {out}");
    assert!(out.contains("--path"), "xml help(shell) should list --path, got: {out}");
    assert!(out.contains("--doc"), "xml help(shell) should list --doc, got: {out}");
}
...
fn json_shell_help_lists_flag_params() {
    ...
    assert!(out.contains("--text"), "json help(shell) should list --text, got: {out}");
    assert!(out.contains("--value"), "json help(shell) should list --value, got: {out}");
}
...
fn fs_shell_help_lists_flag_params() {
    ...
    assert!(out.contains("--path"), "fs help(shell) should list --path, got: {out}");
}
...
fn compress_shell_help_lists_flag_params() {
    ...
    assert!(out.contains("--source"), "compress help(shell) should list --source, got: {out}");
    assert!(out.contains("--archive"), "compress help(shell) should list --archive, got: {out}");
}
...
fn doc_shell_help_lists_flag_params() {
    ...
    assert!(out.contains("--path"), "doc help(shell) should list --path, got: {out}");
    assert!(out.contains("--source"), "doc help(shell) should list --source, got: {out}");
}
...
fn plot_shell_help_lists_flag_params() {
    ...
    assert!(out.contains("--x"), "plot help(shell) should list --x, got: {out}");
    assert!(out.contains("--y"), "plot help(shell) should list --y, got: {out}");
}
...
fn all_modules_shell_help_has_flag_syntax() {
    ...
    let modules = ["xml", "json", "csv", "yaml", "fs", "compress", "doc", "plot"];
