...
fn format_international_default() {
    let s = sb();
    let r = s
        .exec(r#"return phone.format("+12025551234")"#)
        .unwrap();
...
fn is_valid_empty_returns_false() {
    let s = sb();
    let r = s
        .exec(r#"return tostring(phone.isValid(""))"#)
        .unwrap();
    assert_eq!(r, "false");
}
...
fn type_us_number() {
    let s = sb();
    let r = s
        .exec(r#"return phone["type"]("+12025551234")"#)
        .unwrap();
...
fn type_uk_mobile() {
    let s = sb();
    let r = s
        .exec(r#"return phone["type"]("+447911123456")"#)
        .unwrap();
    assert_eq!(r, "mobile");
}
...
fn region_us_number() {
    let s = sb();
    let r = s
        .exec(r#"return phone.region("+12025551234")"#)
        .unwrap();
    assert_eq!(r, "US");
}
...
fn region_uk_number() {
    let s = sb();
    let r = s
        .exec(r#"return phone.region("+442071234567")"#)
        .unwrap();
    assert_eq!(r, "GB");
}
...
fn region_german_number() {
    let s = sb();
    let r = s
        .exec(r#"return phone.region("+4930123456")"#)
        .unwrap();
    assert_eq!(r, "DE");
}
...
fn region_japanese_number() {
    let s = sb();
    let r = s
        .exec(r#"return phone.region("+81312345678")"#)
        .unwrap();
    assert_eq!(r, "JP");
}
...
fn shell_phone_format() {
    ...
    assert!(r.contains("+12025551234"), "expected +12025551234, got: {}", r);
}
