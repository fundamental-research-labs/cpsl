...
fn bare_plot_shows_help() {
    ...
    mt.parse_and_add(&format!("{}:/workspace", dir.path().display())).unwrap();
...
fn xml_dashdash_help_shows_help() {
    let s = sb_with_shell();
    let luau = sh_transpile::transpile_sh("xml --help").unwrap().luau_source;
    let r = s.exec(&luau).unwrap();
...
fn shell_xml_parse_file_dispatch() {
    ...
    let luau = sh_transpile::transpile_sh("xml parseFile /workspace/data.xml")
        ...
        .luau_source;
    let r = s.exec(&luau).unwrap();
    assert!(r.contains("test"), "should display parsed XML file, got: {}", r);
}
...
fn shell_xml_parse_no_args_shows_flag_error() {
    let s = sb_with_shell();
    let luau = sh_transpile::transpile_sh("xml parse")
        .unwrap()
        .luau_source;
    let err = s.exec(&luau).unwrap_err();
    assert!(
...
fn shell_fs_read_no_args_shows_flag_error() {
    let s = sb_with_shell();
    let luau = sh_transpile::transpile_sh("fs read")
        .unwrap()
        .luau_source;
    let err = s.exec(&luau).unwrap_err();
    assert!(
...
fn shell_numx_add_positional() {
    ...
    assert!(
        r.contains("3"),
        "numx add 1 2 should output 3, got: {}",
        r
    );
}
...
fn bare_base64_shows_help() {
    ...
    let r = s.exec(&luau).unwrap();
    assert!(r.contains("base64"), "bare base64 should show help, got: {}", r);
}
