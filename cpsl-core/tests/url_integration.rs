...
fn encode_spaces() {
    let s = sb();
    let r = s
        .exec(r#"return url.encode("hello world")"#)
        .unwrap();
...
fn encode_special_chars() {
    let s = sb();
    let r = s
        .exec(r#"return url.encode("a=1&b=2")"#)
        .unwrap();
...
fn encode_unicode() {
    let s = sb();
    let r = s
        .exec(r#"return url.encode("caf\195\169")"#)
        .unwrap();
...
fn encode_unreserved_chars_not_encoded() {
    let s = sb();
    let r = s
        .exec(r#"return url.encode("a-b_c.d~e")"#)
        .unwrap();
...
fn decode_percent_encoded() {
    let s = sb();
    let r = s
        .exec(r#"return url.decode("hello%20world")"#)
        .unwrap();
...
fn decode_special_chars() {
    let s = sb();
    let r = s
        .exec(r#"return url.decode("a%3D1%26b%3D2")"#)
        .unwrap();
    assert_eq!(r, "a=1&b=2");
}
...
fn decode_unicode() {
    let s = sb();
    let r = s
        .exec(r#"return url.decode("caf%C3%A9")"#)
        .unwrap();
...
fn join_invalid_base_errors() {
    let s = sb();
    let err = s
        .exec(r#"url.join("not-a-url", "/path")"#)
        .unwrap_err();
    assert!(
...
fn shell_url_parse() {
    ...
    let result = cpsl_core::sh_transpile::transpile_sh(
        r#"url parse "https://example.com/path?q=1#frag""#,
    );
...
fn shell_url_join() {
    ...
    s.register_module("shrt", shrt).unwrap();
    let result = cpsl_core::sh_transpile::transpile_sh(
        r#"url join "https://example.com/a/" "../b""#,
    );
    assert!(result.is_ok(), "transpile err: {:?}", result.err());
    let luau = result.unwrap().luau_source;
    let r = s.exec(&luau).unwrap();
...
fn decode_no_percent_encoding() {
    let s = sb();
    let r = s
        .exec(r#"return url.decode("hello")"#)
        .unwrap();
    assert_eq!(r, "hello");
}
