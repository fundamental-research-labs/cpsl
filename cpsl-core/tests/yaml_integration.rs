...
fn decode_table_form() {
    ...
    let s = sb();
    let r = s.exec(r#"local d = yaml.decode({[1]="key: value"}); return d.key"#).unwrap();
    assert_eq!(r, "value");
}
...
fn decode_file_table_form() {
    ...
    let r = s.exec(r#"local d = yaml.decodeFile({[1]="/workspace/data.yaml"}); return d.name"#).unwrap();
    assert_eq!(r, "test");
}
...
fn shell_yaml_decode_roundtrip() {
    ...
    assert!(r.contains("key") && r.contains("value"), "should display parsed yaml, got: {}", r);
}
