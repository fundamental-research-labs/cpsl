...
impl PromptResponse {
    pub fn allow_once() -> Self {
        Self { allowed: true, remember: false }
    }

    pub fn always_allow() -> Self {
        Self { allowed: true, remember: true }
    }

    pub fn deny() -> Self {
        Self { allowed: false, remember: true }
    }
}
...
impl DomainPolicy {
    ...
    pub fn allow(&self, domain: &str) {
        if let Ok(mut set) = self.denied.write() { set.remove(domain); }
        if let Ok(mut set) = self.allowed.write() { set.insert(domain.to_owned()); }
    }

    pub fn deny(&self, domain: &str) {
        if let Ok(mut set) = self.allowed.write() { set.remove(domain); }
        if let Ok(mut set) = self.denied.write() { set.insert(domain.to_owned()); }
    }

    pub fn is_allowed(&self, domain: &str) -> bool {
        self.allowed.read().map(|set| set.contains(domain)).unwrap_or(false)
    }

    pub fn is_denied(&self, domain: &str) -> bool {
        self.denied.read().map(|set| set.contains(domain)).unwrap_or(false)
    }
    ...
    pub fn allowed_domains(&self) -> Vec<String> {
        let Ok(set) = self.allowed.read() else { return Vec::new() };
    ...
    pub fn denied_domains(&self) -> Vec<String> {
        let Ok(set) = self.denied.read() else { return Vec::new() };
        let mut v: Vec<String> = set.iter().cloned().collect();
        v.sort();
        v
    }
...
impl CredentialStore {
    ...
    pub fn remove(&self, domain: &str) {
        if let Ok(mut creds) = self.credentials.write() { creds.remove(domain); }
    }
    ...
    pub fn get(&self, domain: &str) -> Vec<(String, String)> {
        let Ok(creds) = self.credentials.read() else { return Vec::new() };
        // Exact match first
...
mod tests {
    ...
    fn pre_allowed_domain_passes() {
        ...
        assert!(matches!(policy.check("example.com"), DomainVerdict::Allowed));
    }
    ...
    fn wildcard_star_allows_everything() {
        let policy = DomainPolicy::new(None);
        policy.allow("*");
        assert!(matches!(policy.check("anything.com"), DomainVerdict::Allowed));
        assert!(matches!(policy.check("foo.bar.baz"), DomainVerdict::Allowed));
    }
    ...
    fn wildcard_subdomain_matches() {
        let policy = DomainPolicy::new(None);
        policy.allow("*.example.com");
        assert!(matches!(policy.check("api.example.com"), DomainVerdict::Allowed));
        assert!(matches!(policy.check("foo.bar.example.com"), DomainVerdict::Allowed));
        ...
        assert!(matches!(policy.check("notexample.com"), DomainVerdict::Denied));
    }
    ...
    fn wildcard_deny_subdomain() {
        ...
        policy.allow("*");
        policy.deny("*.evil.com");
        assert!(matches!(policy.check("api.evil.com"), DomainVerdict::Denied));
    ...
    fn wildcard_credentials_subdomain() {
        let store = CredentialStore::new();
        store.set("*.api.com", vec![("Authorization".into(), "Bearer x".into())]);
