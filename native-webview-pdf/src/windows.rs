...
fn html_to_pdf_inner(html: &str, opts: &PdfOptions) -> Result<Vec<u8>, PdfError> {
    ...
    let html_wide: Vec<u16> = styled_html.encode_utf16().chain(std::iter::once(0)).collect();
    unsafe {
        webview
...
fn create_environment() -> Result<ICoreWebView2Environment, PdfError> {
    ...
    CreateCoreWebView2EnvironmentCompletedHandler::wait_for_async_operation(
        Box::new(|handler| {
            unsafe { CreateCoreWebView2Environment(&handler) }.map_err(Into::into)
        }),
        Box::new(move |hr, env| {
            hr?;
...
fn create_controller(
    ...
    CreateCoreWebView2ControllerCompletedHandler::wait_for_async_operation(
        Box::new(move |handler| {
            unsafe { env_clone.CreateCoreWebView2Controller(hwnd, &handler) }
                .map_err(Into::into)
        }),
...
fn print_to_pdf_stream(
    ...
    let env6: ICoreWebView2Environment6 = {
        let wv2: ICoreWebView2_2 = webview.cast().map_err(|e| {
            PdfError::PdfGeneration(format!("QI to ICoreWebView2_2: {}", e))
        })?;
        let env_obj = unsafe {
            wv2.Environment()
